...
(function () {
  ...
  /* その山を循環させたときに**実際に順序が変わるか**（rotatePile を実行せずに判定する）。
     公式は「空の山・1種類だけの山を回しても合法（何も起きない）」なので、**回しても何も起きないときは
     窓自体を開かない**（＝天幕/触れ役を使うたびに無意味な確認が出るのを避ける。結果は完全に同一＝
     観測不能なので忠実性は落ちない）。**戦闘計画（任意のサプライ山）は別**＝全部の山を選択肢に出す。 */
  function canRotatePile(state, pileId) {
    if (!pileId) return false;
    const arr = state[pileId];
    if (Array.isArray(arr)) {
      if (arr.length === 0) return false;
      return arr.some((c) => c !== arr[0]);
    }
    const topId = SPLIT_TOP[pileId] || (SPLIT_BOTTOM[pileId] ? pileId : null);
    if (!topId) return false;
    return (state.supply[topId] || 0) > 0 && (state.supply[SPLIT_BOTTOM[topId]] || 0) > 0;
  }
  /* 同盟：「あなたは〈自分の山〉を循環させてもよい」の窓を開く共通入口（薬草集め/古地図/天幕/触れ役/生徒）。
     - **常に任意**＝「回さない」で必ず終端できる（CPU も null を返さない）。
     - `next` を渡すと、循環の解決後にその pending へ進む（生徒＝循環が**先**で、その後に強制廃棄）。
     - 何も起きない循環（空／1種類だけ）は窓を開かず、そのまま next へ進む。
     戻り値＝窓を開いた（＝state.pending を立てた）か。 */
  function openRotateWindow(state, pi, pileId, source, next) {
    if (canRotatePile(state, pileId)) {
      state.pending = { type: 'rotate_pile', player: pi, pile: pileId, source: source || null, next: next || null };
      return true;
    }
    state.pending = next || null;
    return false;
  }
  ...
  /* 好意を n 個得る（A4＝連携カードが配る唯一の入口）。**上限は無い**（マットに積むだけ）。
     ⚠ **好意を配るのは「連携(Liaison)がある王国」だけ**＝Ally が居ないゲーム（state.ally===null）では
        好意は登場しない。連携カードは自分が好意を配るので必ず Ally が居るが、闇市場デッキから連携が出る
        経路（王国10山では判定しない＝§研究doc の「未解決」で「王国10山だけで判定するのが安全」と決めた）
        では Ally が居ないことがある＝その場合は好意を配らない（使い道が無く盤面に幽霊の数字が残るだけ）。 */
  function gainFavors(state, pi, n, note) {
    if (!(n > 0) || !state.ally) return 0;
    const p = state.players[pi];
    p.favors = (p.favors || 0) + n;
    if (note) log(state, `${p.name} は${note} +${n} 好意（計 ${p.favors} 個）。`);
    return n;
  }
  ...
  function playTreasureCard(state, pIndex, card) {
    ...
    if (highwaymanBlocks(state, pIndex)) {
      ...
      /* ⚠ 公式逐語＝`Highwayman does not change anything about the Treasure, just prevents on-play
         instructions on the card from being carried out.` ＝**「そのカードを使った」という事実は残る**。
         止まるのは**そのカード自身の記載効果**だけで、**他のカードの能力**（商人の「最初の銀貨で+$1」・
         私掠船のマーク）や「何枚目か」のカウンタ（愚者の黄金）は普通に働く。
         公式が名指しで例示＝`a blocked Fool's Gold will still let your other Fool's Gold make +[$4]`。 */
      const tt = state.turn;
      if (card === 'silver' && tt && !tt.silverPlayed) {
        if (tt.merchants) { addCoins(state, tt.merchants); log(state, `${p.name} は商人の効果で +${tt.merchants} コイン。`); }
        tt.silverPlayed = true;
      }
      if (card === 'fools_gold' && tt) tt.foolsGoldPlayed = true;
  ...
  function playCardNoAction(state, pi, card, zone, note, way) {
    ...
    /* 同盟：将軍＝「場に2枚以上ある同名のアクションを**手札から**使用できない」。
       ⚠ 公式FAQが玉座の間を名指しで禁止しているとおり、`PLAY_ACTION` 以外の「手札から使わせる」経路
          （長老／専門家／王家のガレー船／市場の町／苦労・進軍）でも止まる。 */
    if (fromHand && warlordBlocks(state, pi, card)) return false;
    ...
    if (isAct) {
      ...
      /* 同盟：追いはぎ＝「他のプレイヤーが**各ターンに最初に使用する財宝**は何もしない」。
         公式FAQ逐語＝`Unlike Enchantress, this can affect a Treasure that another player plays during
         your turn. So if you play Barbarian, trash their Gold, and they gain and play a Buried Treasure,
         the Buried Treasure will do nothing` ＝**手札から購入フェイズに出した財宝に限らない**。
         急使／専門家／薬草集め／博打／苦労 などの「アクション権を消費しない使用」もここを通る。 */
      if (highwaymanBlocks(state, pi)) {
        log(state, `${p.name} の「${C()[card].name}」は追いはぎで何も起こらなかった（このターン最初の財宝）。`);
        const tt = state.turn;
        if (card === 'silver' && tt && !tt.silverPlayed) {
          if (tt.merchants) { addCoins(state, tt.merchants); log(state, `${p.name} は商人の効果で +${tt.merchants} コイン。`); }
        ...
        if (card === 'fools_gold' && tt) tt.foolsGoldPlayed = true;
        corsairOnPlayTreasure(state, pi, card);
        applyCoinPenalty(state);
        return true;
      }
  ...
  function applyTreasureEffect(state, pIndex, card) {
    ...
    /* ===== 略奪P3：財宝 ===== */
    // 宝飾卵＝$1 +1購入（「これを廃棄したとき戦利品1枚」は triggerOnTrash 側＝廃棄した本人が獲得）。
    if (card === 'jewelled_egg') { t.buys += 1; }
    // 縄＝$1 +1購入。次の自分のターンの開始時、+1カード、手札1枚を廃棄してもよい（持続）。
    if (card === 'rope') { t.buys += 1; armDuration(state, pIndex, 'rope'); }
    // 戦利品の袋＝$1 +1購入＋戦利品1枚を獲得（強制）。
    if (card === 'sack_of_loot') {
      t.buys += 1;
      if (!gainLoot(state, pIndex)) log(state, `${p.name} は戦利品の袋を使ったが、戦利品の山が空。`);
    }
    // 坩堝＝手札1枚を廃棄（**強制**）。そのコスト$1につき +$1（**コイン成分だけ**＝ポーション/負債は0扱い＝公式）。
    if (card === 'crucible') {
      if (p.hand.length > 0) state.pending = { type: 'crucible_trash', player: pIndex };
    }
    /* ペンダント＝場に出ている**異なる名前の**財宝1種につき +$1。自分自身も・前ターンからの持続財宝も数える。
       脇置き（貨物船/王子/契約書/資料庫/操舵手）は数えない。「財宝か」は isTreasureFor（資本主義対応）。
       金額は**使用時に確定**（後から場が変わっても足し直さない＝公式）。 */
    if (card === 'pendant') {
      const kinds = new Set();
      p.inPlay.concat(p.durationCards || []).forEach((c) => { if (isTreasureFor(state, c)) kinds.add(c); });
    ...
    if (card === 'silver_mine') {
      const ref = costOf(state, 'silver_mine');
      if (anyGainable(state, (id) => gainableBase(state, id) && isTypeSupply(state, id, 'treasure') && costLT(costOf(state, id), ref))) {
        state.pending = { type: 'silver_mine_gain', player: pIndex };
      } else {
        log(state, `${p.name} は銀山を使ったが、これより安い財宝がサプライに無い。`);
      }
    }
    /* つるはし＝$1＋手札1枚を廃棄（**強制**）。廃棄した**後の**コストが3以上なら、戦利品1枚を**手札に**獲得
       （コスト参照は廃棄の後＝漁師が捨て札ありで$5になる例・必読J）。 */
    if (card === 'pickaxe') {
      if (p.hand.length > 0) state.pending = { type: 'pickaxe_trash', player: pIndex };
    }
    // 小像＝+2カード。手札のアクションカード1枚を捨ててもよい→捨てたら +1購入 +$1（捨て札トリガーを通す）。
    if (card === 'figurine') {
      draw(state, pIndex, 2);
      if (p.hand.some((c) => DOM.isType(c, 'action'))) state.pending = { type: 'figurine_discard', player: pIndex };
    }
    /* 工具＝（自分を含む）誰かが場に出しているのと同じカード1枚を獲得（強制）。
       工具自身が場にあるので候補は常に1つ以上ある。サプライに無い札（戦利品等）は選べるが獲得は起きない（公式）。 */
    if (card === 'tools') {
      state.pending = { type: 'tools_gain', player: pIndex };
    }
    // ゴンドラ＝「今 +$2」か「次の自分のターンの開始時 +$2」を選ぶ（プレイのたびに独立に選ぶ＝アンフォラと同型）。
    if (card === 'gondola') {
      state.pending = { type: 'gondola_choose', player: pIndex };
    }
    /* 埋められた財宝＝次の自分のターンの開始時 +1購入 +$3。
    ...
    /* ===== 同盟（Allies）A4：財宝（効果は必ずここに書く＝applyEffect は財宝では呼ばれない） ===== */
    /* 沈没船の財宝＝**コインは出さない（$0）**。「自分の場に同名のコピーが無いアクションカード」を
       1枚獲得する（**強制・コスト制限なし**）。該当が無ければ何も獲得しない（公式FAQ逐語）。
       ⚠ コスト述語（costUpTo 等）を使ってはいけない＝コスト制限ではない。 */
    if (card === 'sunken_treasure') {
      if (anyGainable(state, sunkenTreasureCanGain(state, pIndex))) {
        state.pending = { type: 'sunken_treasure', player: pIndex };
      }
    }
    /* 道化棒（財宝・連携）＝4つから**異なる2つ**を選ぶ（+1購入／+1コイン／+1好意／
       「このターン、カードを獲得するとき山札の上に置いてよい」）。**先に2つ選んでから解決する**（公式）。
       ⚠ **再演（冠/ティアラ/偽造通貨/専門家）のたびに選び直せる**＝命令の commandAs を流用しない。
       ⚠ 4つ目は "this turn," なので**場を離れても効き続ける**（偽造通貨で廃棄しても有効＝公式）。 */
    if (card === 'bauble') state.pending = { type: 'bauble_choose', player: pIndex, picked: [] };
    /* 契約書（財宝・持続・連携）＝+$2（coin:2 で計上済み）+1好意 →
       手札のアクション1枚を脇に置いてよい（任意）。**置いたときだけ持続になる**（貨物船と同型）。
       置いた札は**表向き**＝公開情報。次の自分のターンの開始時に**強制で使用する**。 */
    if (card === 'contract') {
      gainFavors(state, pIndex, 1, 'は契約書で');
      if (p.hand.some((c) => DOM.isType(c, 'action') || inheritedEstate(p, c))) {
        state.pending = { type: 'contract_setaside', player: pIndex };
      }
    }
  ...
  /* 同盟：密使（Emissary）＝「**このカードで**（少なくとも1枚を）シャッフルした場合」を判定するための
     シャッフル回数カウンタ。reduce は同期的に走るので、効果の前後で差分を取れば「その効果の解決中に
     シャッフルが起きたか」が分かる（`applyPileTokens` 等の**効果解決より前**の区間を含めないこと）。
     ⚠ 公式FAQ逐語＝`It only counts as shuffling if at least one card was in your discard pile.`
  ...
  function freshTurn(active, isExtraTurn, extra) {
    ...
    return {
      ...
      /* 同盟 A4：「**このターン**、〜したとき」型の設置（"while this is in play" ではないので
         場を離れても効き続け、**使用回数ぶん累積する**）。庭師(groundskeeper)の「場の枚数」とは別機構。 */
      guildmasters: 0,              // ギルドマスター＝獲得のたびに +1好意（使用回数ぶん）
      galleria: 0,                  // ガレリア＝ちょうど$3/$4 の獲得で +1購入（使用回数ぶん）
      baubleTopdeck: 0,             // 道化棒＝獲得したカードを山札の上に置いてよい（移動遊園地と同じ窓）
      skirmishers: null,            // 散兵＝アタックカードの獲得で相手が手札3枚まで捨てる（使用回数ぶん・免疫は使用時に確定）
      garrisonTokens: null,         // 駐屯地＝場のインスタンスごとのトークン数（**好意ではない**）
      topdeckDone: false,           // 天幕/商人の野営地の「場から捨てる代わりに山札の上へ」窓を出したか
  ...
  function createInitialState(playerConfigs, kingdom, opts) {
    ...
    /* 略奪：シャーマン＝**ゲームの最初のターンにも**「ターンの開始時」は起きる（公式＝
       ...
       廃棄置き場に最初からゾンビ3枚がある＝ターン1から獲得が発生する。 */
    if (kingdom.includes('shaman') && shamanTargets(initial).length) {
      initial.turn.startQueue = initial.turn.startQueue || [];
      initial.turn.startQueue.push({ type: 'shaman_gain', player: startActive });
    }
  ...
  function trashCard(state, ownerIdx, card, opts) {
    ...
    if (opts && opts.deferTrigger) return true; // 複数枚を同時に廃棄する経路（trashCardsTogether）が後でまとめて誘発する
    return triggerOnTrash(state, ownerIdx, card, opts); // 城塞は手札へ戻り false／nomads等の副次効果も発動
  }
  /* 複数枚を**同時に**廃棄する（＝**全部を廃棄置き場に入れてから** on-trash を解決する）。
     公式逐語（同盟・リッチ）＝`Sentinel で Lich と Sycophant を同時に廃棄すると、Lich はその Sycophant を獲得できる`
     ＝1枚ずつ `trashCard` を呼ぶと、送った配列の順番で結果が変わってしまう。
     ⚠ **現状これを使っているのは歩哨だけ**。礼拝堂/神殿/平和的教団など他の複数枚廃棄は
        従来どおり1枚ずつ on-trash を解決する（既存の横断挙動＝PROGRESS に許容簡略化として記録）。 */
  function trashCardsTogether(state, ownerIdx, cards, opts) {
    const list = (cards || []).slice();
    list.forEach((c) => trashCard(state, ownerIdx, c, Object.assign({}, opts || {}, { deferTrigger: true })));
    list.forEach((c) => triggerOnTrash(state, ownerIdx, c, opts));
  }
  ...
  function sirenEnterVictim(state, source, queue) {
    ...
    if (hasReaction(state.players[victim])) {
      state.pending = { type: 'siren', stage: 'react', player: victim, source, victim, queue: rest };
    } else {
      sirenCurse(state, source, victim, rest);
    }
  }
  function sirenCurse(state, source, victim, queue) {
  ...
  /* シャーマン＝廃棄置き場のコスト6以下（コイン≤6・ポーション0・負債0）のカード。
     **engine 拒否・CPU 候補・UI フィルタが必ずこの述語を見る**（非サプライなので costUpTo は使えない＝
     リッチ lichTrashTargets と同型）。 */
  function shamanTargets(state) {
    const out = [];
    (state.trash || []).forEach((c) => {
      if (out.indexOf(c) >= 0 || !C()[c]) return;
      if (costLE(costOf(state, c), { coin: 6, pot: 0, debt: 0 })) out.push(c);
    });
    return out;
  }
  /* 工具＝（自分を含む）誰かが場に出しているカードの id 一覧（重複なし）。
     公式＝「サプライに無いカード（戦利品など）も**選べる**が、獲得は起きない」＝候補からは消さない
     （探索/専門家と同じ「遂行できない選択肢も選べる」方針。UI はラベルで補う）。 */
  function toolsTargets(state) {
    const out = [];
    state.players.forEach((pl) => {
      pl.inPlay.concat(pl.durationCards || []).forEach((c) => { if (out.indexOf(c) < 0 && C()[c]) out.push(c); });
    });
    return out;
  }
  /* ===== 移動動物園：追放（Exile）=====
  ...
  function applyWay(state, wayId, card, pi) {
    const t = state.turn, p = state.players[pi];
    /* 略奪P3：港の村＝習性(Way)として使った場合も「次のアクションの使用」なので予約は消費するが、
       **習性由来の +$ ではボーナスを得ない**（決定D3＝2025年2月の移動動物園エラッタ側）。
       カメレオンの習性だけは公式には得られるが、極小の mix-all 限定差＝許容簡略化（PROGRESS に記録）。 */
  ...
  /* 略奪P3：港の村＝「このターン次に使用したアクションが**その効果で** +$ を出していたら +$1」。
     applyEffect（＝アクションの使用・再演のすべての入口）をラップし、予約（t.hvWatchers[pi]）を消費して
     効果の前後のコイン差で判定する。財源(+Coffers)・山トークン・チャンピオン等は applyEffect の外＝自然に除外。
     効果が選択待ちを立てた場合（執事の+$2等）は解決後（reduce 末尾の hvPending 精算）で判定する。
     習性(Way)由来の +$ は applyWay 側で予約だけ消費しボーナスを出さない（決定D3）。 */
  function applyEffect(state, cardId, pi) {
    const t0 = state.turn;
    let hvN = 0, hvC0 = 0;
    if (t0 && t0.hvWatchers && t0.hvWatchers[pi] > 0) {
      hvN = t0.hvWatchers[pi]; t0.hvWatchers[pi] = 0; hvC0 = t0.coins || 0;
    }
    const r = applyEffectInner(state, cardId, pi);
  ...
  function settleHarborVillage(state, pi, n, coins0) {
    const t = state.turn;
    if (!t || pi !== t.active) return; // 自分のターンでなければコインは得られない
    if ((t.coins || 0) > coins0) {
      addCoins(state, n);
      log(state, `${state.players[pi].name} は港の村で +$${n}（次のアクションが +$ を出した）。`);
    }
  }
  function applyEffectInner(state, cardId, pi) {
    const t = state.turn;
    const p = state.players[pi];
    switch (cardId) {
      ...
      /* ===== 略奪（Plunder）P3：王国カード ===== */
      /* 岩屋＝+1アクション。手札を最大4枚（この上に）伏せて置いてもよい。次の自分のターンの開始時、
         それらを**捨て札にし、その後**同じ枚数を引く（公式＝捨てる→引くの順・0枚なら持続にならない）。 */
      case 'grotto':
        addActions(t, 1);
        if (p.hand.length > 0) state.pending = { type: 'grotto_set', player: pi };
        break;
      /* シャーマン＝+1アクション +$1、手札1枚を廃棄してもよい。
         （ライン下＝「シャーマンを使うゲームでは各ターンの開始時に廃棄置き場からコスト6以下を獲得」は
          resolveDurationStartEffects／createInitialState 側＝**王国にあるだけで全員に効く**。） */
      case 'shaman':
        addActions(t, 1); addCoins(state, 1);
        if (p.hand.length > 0) state.pending = { type: 'shaman_trash', player: pi };
        break;
      /* セイレーン＝他の全員が呪い1枚を獲得（アタック・使い魔と同型の連鎖）。
         次の自分のターンの開始時、手札が8枚になるようにカードを引く（持続）。 */
      case 'siren': {
        armDuration(state, pi, 'siren');
        const vics = [];
        for (let k = 1; k < state.players.length; k++) vics.push((pi + k) % state.players.length);
        sirenEnterVictim(state, pi, vics);
        break;
      }
      // 密航者＝次の自分のターンの開始時 +2カード（「誰かが持続を獲得したとき手札から使用」は triggerOnGain 側）。
      case 'stowaway':
        armDuration(state, pi, 'stowaway');
        break;
      /* 現場監督＝+1アクション +$1。このターン、**これより後に**コストちょうど$5（コイン5・ポーション0・負債0）の
         カードを獲得した場合、次のターンの開始時にこの能力を冒頭から繰り返す。
         ⚠ 「繰り返し」は「アクションの使用」ではない（必読11c）＝noteAllyPlay も actionsPlayed も触らない。 */
      case 'taskmaster':
        addActions(t, 1); addCoins(state, 1);
        (t.taskmasterWatch = t.taskmasterWatch || []).push({ player: pi, hit: false });
        break;
      // キャビンボーイ＝+1カード +1アクション。次のターンの開始時に「+$2」か「これを廃棄して持続カード獲得」。
      case 'cabin_boy':
        draw(state, pi, 1); addActions(t, 1);
        armDuration(state, pi, 'cabin_boy');
        break;
      /* 置き去り＝手札1枚を廃棄（**強制**）。それが持つ種別1つにつき +2カード。
         種別は**静的な types**（特性や災いカードは種別を増やさない＝公式）。手札0枚なら廃棄に失敗＝何も起きない。 */
      case 'maroon':
        if (p.hand.length > 0) state.pending = { type: 'maroon_trash', player: pi };
        break;
      /* 沼地の小屋＝+2アクション。場のカード3枚（端数切り捨て）につき +1カード。
         自分の inPlay＋durationCards（前ターンからの持続も数える＝公式）。脇置き（貨物船/王子/操舵手）は数えない。 */
      case 'swamp_shacks': {
        addActions(t, 2);
        const inPlayN = p.inPlay.length + (p.durationCards || []).length;
        const n = Math.floor(inPlayN / 3);
        if (n > 0) draw(state, pi, n);
        log(state, `${p.name} は沼地の小屋で +${n}カード（場に${inPlayN}枚）。`);
        break;
      }
      /* 港の村＝+1カード +2アクション。このターン次に使用したアクションが**その効果で** +$ を出していたら +$1。
         判定は applyEffect のラッパ（hvWatchers 消費→コイン差）＋選択待ちを挟む効果は reduce 末尾の hvPending 精算。
         習性(Way)由来の +$ ではボーナスを得ない（決定D3）。 */
      case 'harbor_village':
        draw(state, pi, 1); addActions(t, 2);
        if (!t.hvWatchers) t.hvWatchers = {};
        t.hvWatchers[pi] = (t.hvWatchers[pi] || 0) + 1;
        break;
      // 巡礼者＝+4カード、手札1枚を山札の上に置く（**引けなくても強制**・候補は手札全部＝公式）。
      case 'pilgrim':
        draw(state, pi, 4);
        if (p.hand.length > 0) state.pending = { type: 'pilgrim_put', player: pi };
        break;
      // ロングシップ＝+2アクション。次の自分のターンの開始時 +2カード。
      case 'longship':
        addActions(t, 2);
        armDuration(state, pi, 'longship');
        break;
      // 価値ある村＝+1カード +2アクション（「獲得時＝場に財宝3種以上なら戦利品」は triggerOnGain 側）。
      case 'wealthy_village':
        draw(state, pi, 1); addActions(t, 2);
        break;
      // 乗組員＝+3カード。次の自分のターンの開始時、これを場から山札の上に置く（強制・捨て札置き場を経由しない）。
      case 'crew':
        draw(state, pi, 3);
      ...
      /* =====================================================================
         ...
         正本＝docs/research/allies_rules.md。落とし穴は PROGRESS §0-29 の注意節。
         ⚠ 連携(Liaison)が配る好意は必ず `gainFavors` を通す（Ally が居ないゲームでは配らない）。
         ⚠ 循環(Rotate)は `openRotateWindow`（**常に任意**・位置はカードごとに違う）。
         ⚠ Ally が起こす攻撃と違い、**王国のアタック7種は普通のアタック**＝ATTACKS 登録＋堀で防げる。
         ===================================================================== */
      // 商人の野営地：+2アクション +1コイン（「場から捨てるとき山札の上に置いてよい」は片付け側＝ALLIES_TOPDECK）。
      case 'merchant_camp':
        addActions(t, 2); addCoins(state, 1);
        break;
      // 下役：+1カード +1アクション +1好意（連携）。
      case 'underling':
        draw(state, pi, 1); addActions(t, 1);
        gainFavors(state, pi, 1, 'は下役で');
        break;
      ...
      case 'miller': {
        ...
        if (look.length) state.pending = { type: 'miller_pick', player: pi, cards: look };
        break;
      }
      /* 侯爵：+1購入、手札1枚につき +1カード、その後 手札が10枚になるように捨てる。
         引く枚数は「これが手札を離れた後の手札枚数」＝場に出ているので自然にそうなる。 */
      case 'marquis':
        t.buys += 1;
        if (p.hand.length > 0) draw(state, pi, p.hand.length);
        if (p.hand.length > 10) state.pending = { type: 'marquis_discard', player: pi };
        break;
      /* ごますり（連携）：+1アクション、カード3枚を捨てる（**強制**・手札が3枚未満ならあるだけ）。
         1枚以上捨てたら +3コイン。
         ⚠ **3枚は同時に捨てる**（1枚ずつにすると坑道→望楼の連鎖で不正が通る＝公式FAQ逐語）。
         好意は「獲得または廃棄するとき」＝triggerOnGain / triggerOnTrash 側。 */
      case 'sycophant':
        addActions(t, 1);
        if (p.hand.length > 3) state.pending = { type: 'sycophant_discard', player: pi };
        else if (p.hand.length > 0) sycophantDiscard(state, pi, p.hand.slice()); // 選択の余地なし＝自動で全部捨てる
        break;
      /* 首都：+1カード +2アクション。手札2枚を捨てて +2コイン してもよい（任意）。
         その後 2コインを払って +2カード してもよい（任意）。**順序はカード記載順**（捨てて得たコインで払える）。
         公式FAQ逐語＝`You may choose to do this even with fewer than 2 cards in hand.`（＝遂行できなくても選べる）。 */
      case 'capital_city':
        draw(state, pi, 1); addActions(t, 2);
        state.pending = { type: 'capital_city', stage: 'discard', player: pi };
        break;
      /* 宿屋の主人：+1アクション → **引く前に**三択（+1カード ／ +3カード後に3枚捨て ／ +5カード後に6枚捨て）。
      ...
      case 'hunter': {
        ...
        if (look.length) {
          reveal(state, pi, look, '狩人で山札の上3枚を公開');
          hunterStart(state, pi, look);
        }
        break;
      }
      // 女予言者：+4カード +1アクション。手札1枚を山札の上に、もう1枚を山札の一番下に置く（強制・引いた後）。
      case 'sibyl':
        draw(state, pi, 4); addActions(t, 1);
        if (p.hand.length > 0) state.pending = { type: 'sibyl_place', stage: 'top', player: pi };
        break;
      // 遠い海岸：+2カード +1アクション、屋敷1枚を獲得（強制）。2勝利点。
      case 'distant_shore':
        draw(state, pi, 2); addActions(t, 1);
        if (gain(state, pi, 'estate', 'discard')) log(state, `${p.name} は遠い海岸で屋敷1枚を獲得した。`);
        break;
      ...
      case 'tent':
        addCoins(state, 2);
        openRotateWindow(state, pi, 'forts', 'tent', null);
        break;
      /* 駐屯地：+2コイン。**このターン**カードを獲得するたび、この物理カードの上にトークン1個。
         次の自分のターン開始時、トークンを全部取り除き1個につき +1カード。
         ⚠ トークンが0個ならクリンナップで普通に捨てる（**条件つき持続**）＝片付け時に判定する。
         ⚠ トークンは**場のインスタンス単位**（玉座で2回使うと1獲得につき2個・除去は1回）＝
            `t.garrisonTokens` に「場に出ている駐屯地の“プレイ回数”ぶんの配列」を持つ。
         ⚠ **好意ではない**（カードの上に載るだけの自前カウンタ）。 */
      case 'garrison':
        addCoins(state, 2);
        (t.garrisonTokens = t.garrisonTokens || []).push(0);
        break;
      /* 堡塁：コスト4以下を1枚獲得（強制）→ **獲得時効果を全部解決してから** 二択
         （獲得したカードを手札に加える ／ +1カード +1アクション）。
      ...
      case 'modify':
        ...
        // 手札が空＝廃棄できず「これ(it)」も存在しない＝二択は行うが**獲得は何も起きない**（`noTrash`）。
        else state.pending = { type: 'modify_choose', player: pi, noTrash: true, coin: 0, pot: 0, debt: 0, elder: elderOn(state, 'modify') };
        break;
      ...
      case 'broker':
        if (p.hand.length > 0) state.pending = { type: 'broker_trash', player: pi };
        break;
      /* ギルドマスター（連携）：+3コイン。「**このターン**、カード1枚を獲得するとき +1好意」。
         "This turn," なので**場を離れても効き続け、使用回数ぶん累積する**（triggerOnGain が読む）。 */
      case 'guildmaster':
        addCoins(state, 3);
        t.guildmasters = (t.guildmasters || 0) + 1;
        break;
      // ガレリア：+3コイン。「このターン、**ちょうど$3か$4**のカードを獲得するとき +1購入」（累積）。
      case 'galleria':
        addCoins(state, 3);
        t.galleria = (t.galleria || 0) + 1;
        break;
      /* 密使（連携）：+3カード。**このカードの +3カード でシャッフルが起きたら** +1アクション +2好意。
         ⚠ 山トークン（Pathfinding）等の**効果解決より前**のドローで起きたシャッフルは数えない
            （公式FAQ逐語）＝ここ（case に入ってから）で前後差を取る。 */
      case 'emissary': {
        const before = shuffleTicks;
        draw(state, pi, 3);
        if (shuffleTicks > before) {
          addActions(t, 1);
          gainFavors(state, pi, 2, 'は密使のシャッフルで');
          log(state, `${p.name} は密使でシャッフルした（+1アクション）。`);
        }
        break;
      }
      /* 輸入者（連携・持続）：使用したターンは何もしない。**次の自分のターンの開始時に $5以下を1枚獲得**（強制）。
         準備の「各プレイヤー +4好意」は createInitialState（A1 で実装済み）。 */
      case 'importer':
        armDuration(state, pi, 'importer');
        break;
      /* 生徒（連携・魔法使い）：+1アクション → **魔法使いの山を循環してよい（先！）** → 手札1枚を廃棄（強制）。
         廃棄したのが財宝なら +1好意 かつ **これ（場の生徒）を山札の上に置く**（両方とも強制）。
         ⚠ 循環は「常に最後」ではない＝この1枚だけ廃棄より前（公式カード文の行順）。 */
      case 'student':
        addActions(t, 1);
        openRotateWindow(state, pi, 'wizards', 'student',
          p.hand.length > 0 ? { type: 'student_trash', player: pi } : null);
        break;
      ...
      /* 薬草集め（卜占官）：+1購入 → **山札を全部 捨て札に置く**（⚠ これは「捨てる」ではないので
         ...
         使用してよい（任意）→ 卜占官の山を循環してよい（任意・最後）。 */
      case 'herb_gatherer': {
        t.buys += 1;
        const nDeck = p.deck.length;
        while (p.deck.length) p.discard.push(p.deck.shift());
        if (nDeck) log(state, `${p.name} は薬草集めで山札${nDeck}枚を捨て札に置いた（捨て札トリガーは誘発しない）。`);
        if (p.discard.some((c) => isTreasureFor(state, c))) state.pending = { type: 'herb_gatherer_play', player: pi };
        else openRotateWindow(state, pi, 'augurs', 'herb_gatherer', null);
        break;
      }
      /* 古地図（叙事詩）：+1カード +1アクション → 手札1枚を捨てる（**強制**・捨て札トリガーは誘発する）→
         +1カード → 叙事詩の山を循環してよい（任意・最後）。公式FAQ逐語＝`Everything happens in the order listed`。 */
      case 'old_map':
        draw(state, pi, 1); addActions(t, 1);
        if (p.hand.length > 0) state.pending = { type: 'old_map_discard', player: pi };
        else { draw(state, pi, 1); openRotateWindow(state, pi, 'odysseys', 'old_map', null); }
        break;
      /* 戦闘計画（衝突）：+1カード +1アクション → 手札のアタックカード1枚を公開してよい（+1カード）→
         **任意のサプライ山**を循環してよい（`rotatableSupplyPiles` が正本＝騎士/廃墟/城/サウナも対象）。 */
      case 'battle_plan':
        draw(state, pi, 1); addActions(t, 1);
        if (p.hand.some((c) => DOM.isType(c, 'attack'))) state.pending = { type: 'battle_plan_reveal', player: pi };
        else state.pending = { type: 'rotate_pile', player: pi, any: true, source: 'battle_plan' };
        break;
      /* 領土（勝利点・衝突）＝可変VP（所持する**異なる名前の勝利点カード1種類につき1点**・自身も数える）は vpOf。
  ...
  function vpOf(p) {
    ...
    /* 同盟：領土＝所持する**異なる名前の勝利点カード1種類につき1勝利点**（領土自身も1種類として数える。
       枚数ではなく名前の種類数＝屋敷を3枚持っていても屋敷は1種類）。領土1枚ごとにその点数。 */
    const territories = cards.filter((c) => c === 'territory').length;
    if (territories) {
      const names = new Set();
      cards.forEach((c) => { if (C()[c] && DOM.isType(c, 'victory')) names.add(c); });
      vp += territories * names.size;
    }
  ...
  /* ===== 同盟 A4：王国カードの共通ヘルパ =====
     手札から複数枚を「同時に」捨てる（捨て札トリガーは全部移し終えてから解決する＝公式）。
     ⚠ 1枚ずつ「捨てる→トリガー→次を選ぶ」にすると坑道→望楼の連鎖で不正が通る（ごますりの公式FAQ逐語）。 */
  function alliesDiscardHand(state, pi, cards, note) {
    const p = state.players[pi];
    const moved = [];
    (cards || []).forEach((c) => { if (removeOne(p.hand, c)) { p.discard.push(c); moved.push(c); } });
    if (moved.length && note) log(state, `${p.name} は${note}手札${moved.length}枚を捨てた。`);
    if (moved.length) triggerOnDiscard(state, pi, moved);
    return moved;
  }
  /* 沈没船の財宝＝「自分の場に同名のコピーを出していないアクションカード」の候補述語。
     **コスト制限は無い**（$8 の王の宮廷でも獲得できる）＝`costUpTo` を掛けてはいけない。
     「場にある」は inPlay ＋ durationCards の両方を見る（持続で残っている札も「場にある」）。
     engine拒否・CPU候補・UIフィルタの3面が同じ述語を見る。 */
  function sunkenTreasureCanGain(state, pi) {
    const p = state.players[pi];
    const inPlay = new Set((p.inPlay || []).concat(p.durationCards || []));
    return (id) => gainableBase(state, id) && isTypeSupply(state, id, 'action') &&
      !inPlay.has(mixedTopCard(state, id) || id);
  }
  ...
  /* 「その手札の1枚を今このプレイヤーが使えるか」＝航海の3枚制限 ＋ 将軍の同名2枚制限。
     **engine のゲート・CPU の候補・UI のフィルタが必ずこれを見る**（片側だけだと死に選択肢／本番 livelock）。 */
  function canPlayHandCard(state, pi, card) {
    return canPlayFromHand(state, pi) && !warlordBlocks(state, pi, card);
  }
  ...
  /* ===== 同盟：長老（Elder）＝「選ぶ」で能力を選ぶとき、追加で異なるもの1つを選んでよい =====
     ...
     - `t.elderBoost` に「長老が使わせたカードid」を入れる。**そのプレイにだけ**効かせるため、
       `PLAY_ACTION`（自分でアクションを使い始めた）と `runReplays`（再演）で落とす
       ＝同名の2枚目を普通に使っても追加選択は付かない／御料車・王笏の再演にも付かない（公式FAQ逐語）。
  ...
  function applyChoiceOption(state, kind, pi, opt, ctx) {
    ...
    if (kind === 'modify') {
      ...
      // ⚠ 手札が空で1枚も廃棄していないときは「これ(it)」が存在しない＝**何も獲得しない**（`noTrash`）。
      else if (ctx && !ctx.noTrash && anyGainable(state, modifyCanGain(state, ctx))) {
  ...
  /* 狩人＝公開した3枚から **アクション → 財宝 → 勝利点 の順**に1枚ずつ手札へ（強制）。
     ...
     候補が1枚だけなら自動で取る／全部の枠が終わったら残りを捨てる。 */
  const HUNTER_STAGES = ['action', 'treasure', 'victory'];
  function hunterMatch(state, c, stage) {
    if (stage === 'treasure') return isTreasureFor(state, c);
    return DOM.isType(c, stage);
  }
  function hunterAdvance(state, pd) {
    const p = state.players[pd.player];
    let si = HUNTER_STAGES.indexOf(pd.stage);
    for (si += 1; si < HUNTER_STAGES.length; si++) {
      const st = HUNTER_STAGES[si];
      const cands = pd.cards.filter((c) => hunterMatch(state, c, st));
      if (cands.length === 0) continue;              // 該当なし＝その枠は空
      if (cands.length === 1) {                      // 選択の余地なし＝自動
        removeOne(pd.cards, cands[0]);
        p.hand.push(cands[0]);
        log(state, `${p.name} は狩人で「${C()[cands[0]].name}」を手札に加えた。`);
        continue;
      }
      state.pending = { type: 'hunter_pick', player: pd.player, cards: pd.cards, stage: st };
      return;
    }
    // 全部の枠が終わった＝残りを捨てる
    if (pd.cards.length) {
      ...
      log(state, `${p.name} は狩人で残り${rest.length}枚を捨てた。`);
      triggerOnDiscard(state, pd.player, rest);
    }
    if (!state.pending || state.pending.type === 'hunter_pick') state.pending = null;
  }
  // 狩人：最初の枠へ入る（stage=null から始める）。
  function hunterStart(state, pi, cards) {
    hunterAdvance(state, { player: pi, cards, stage: null });
  }
  ...
  const DURATION_RESOLVERS = {
    ...
    /* ===== 略奪P3 ===== */
    // 岩屋＝脇の札を**捨て札にし、その後**同じ枚数を引く（捨て札トリガーを先に解決＝坑道の金貨がリシャッフルに入る）。
    grotto: (s, pi, e) => {
      const pl = s.players[pi];
      const back = (e.stashed || []).filter((c) => removeOne(pl.setAside, c));
      if (back.length) {
        back.forEach((c) => pl.discard.push(c));
        log(s, `${pl.name} は岩屋で脇の${back.length}枚を捨て札にした。`);
        triggerOnDiscard(s, pi, back);
        draw(s, pi, back.length);
        log(s, `${pl.name} は岩屋で +${back.length}カード。`);
      }
    },
    // 縄＝+1カード、その後 手札1枚を廃棄してもよい（任意＝対話は startQueue へ）。
    rope: (s, pi) => {
      draw(s, pi, 1);
      log(s, `${s.players[pi].name} は縄の持続効果（+1カード）。`);
      if (s.players[pi].hand.length > 0) s.turn.startQueue.push({ type: 'rope_trash', player: pi });
    },
    // ロングシップ＝+2カード。
    longship: (s, pi) => { draw(s, pi, 2); log(s, `${s.players[pi].name} はロングシップで +2カード。`); },
    // ゴンドラ＝（「次のターンの開始時」を選んだぶん）+$2。
    gondola: (s, pi) => { addCoins(s, 2); log(s, `${s.players[pi].name} はゴンドラの持続効果（+$2）。`); },
    // 埋められた財宝＝+1購入 +$3。
    buried_treasure: (s, pi) => { s.turn.buys += 1; addCoins(s, 3); log(s, `${s.players[pi].name} は埋められた財宝の持続効果（+1購入 +$3）。`); },
    // セイレーン＝手札が8枚になるようにカードを引く。
    siren: (s, pi) => {
      const pl = s.players[pi];
      const n = Math.max(0, 8 - pl.hand.length);
      if (n > 0) draw(s, pi, n);
      log(s, `${pl.name} はセイレーンで手札が8枚になるように引いた（+${n}）。`);
    },
    // 密航者＝+2カード。
    stowaway: (s, pi) => { draw(s, pi, 2); log(s, `${s.players[pi].name} は密航者で +2カード。`); },
    /* 乗組員＝これを場から**山札の上**に置く（強制・捨て札置き場を経由しない＝捨て札トリガーを通さない）。
       既に場を離れていれば何も起きない（lose track）。 */
    crew: (s, pi) => {
      const pl = s.players[pi];
      if (removeOne(pl.durationCards || [], 'crew') || removeOne(pl.inPlay, 'crew')) {
        pl.deck.unshift('crew');
        log(s, `${pl.name} は乗組員を山札の上に置いた。`);
      }
    },
    /* 現場監督＝能力を冒頭から繰り返す（+1アクション +$1＋このターンの$5獲得をまた見張る）。
       「繰り返し」は**アクションの使用ではない**＝noteAllyPlay・actionsPlayed・山トークンは触らない（必読11c）。 */
    taskmaster: (s, pi) => {
      addActions(s.turn, 1); addCoins(s, 1);
      (s.turn.taskmasterWatch = s.turn.taskmasterWatch || []).push({ player: pi, hit: false });
      log(s, `${s.players[pi].name} は現場監督の能力を繰り返した（+1アクション +$1）。`);
    },
    ...
    /* 同盟：契約書＝脇に置いたアクションを次の自分のターンの開始時に**使用する**（強制・アクション権不要）。
       ⚠ 契約書自身はそのターンの片付けで捨てられる（脇札が持続でも＝公式の明示例外）。 */
    contract: (s, pi) => {
      const pl = s.players[pi];
      if ((pl.contractSetAside || []).length) s.turn.startQueue.push({ type: 'contract_play', player: pi });
    },
    /* 同盟：要塞＝「次のターンの開始時に +3カード」を選んだときだけ持続になる（条件つき持続）。 */
    stronghold: (s, pi) => { draw(s, pi, 3); log(s, `${s.players[pi].name} は要塞の持続効果（+3カード）。`); },
    /* 同盟：駐屯地＝次の自分のターン開始時、上のトークンを全部取り除き1個につき +1カード（強制）。
       トークンが0個ならそもそも予約を張らない（＝片付けで普通に捨てる＝条件つき持続）。 */
    garrison: (s, pi, e) => {
      const n = e.n || 0;
      if (n > 0) { const got = draw(s, pi, n); log(s, `${s.players[pi].name} は駐屯地のトークン${n}個を取り除いて +${got.length}カード。`); }
    },
  ...
  function triggerOnGain(state, pIndex, cardId, dest) {
    ...
    /* 略奪P3：現場監督＝「これより後に」コストちょうど$5（コイン5・ポーション0・負債0）のカードを獲得したか。
       1回の$5獲得で、その時点で見張っている自分の予約すべてが満たされる（以後の獲得は不要）。 */
    if (state.turn && (state.turn.taskmasterWatch || []).length) {
      const tmCost = costOf(state, cardId);
      if (tmCost.coin === 5 && !tmCost.pot && !tmCost.debt) {
        state.turn.taskmasterWatch.forEach((w) => { if (w.player === pIndex && !w.hit) w.hit = true; });
      }
    }
    for (let o = 0; o < n; o++) {
    ...
    /* ===== 略奪P3：獲得時の効果 ===== */
    /* 価値ある村＝これを獲得したとき、異なる財宝カードを3種類以上**場に**出していたら戦利品1枚を獲得
       （自動・非対話。「獲得時の場」を見る＝アクションフェイズの獲得ではほぼ空振り＝公式）。 */
    if (cardId === 'wealthy_village') {
      const kinds = new Set();
      gp.inPlay.concat(gp.durationCards || []).forEach((c) => { if (isTreasureFor(state, c)) kinds.add(c); });
      if (kinds.size >= 3) {
        log(state, `${gp.name} は価値ある村の獲得時、場の財宝が${kinds.size}種類あるので戦利品を獲得する。`);
        gainLoot(state, pIndex);
      }
    }
    /* 埋められた財宝＝これを獲得したとき、**使用する**（強制・誰のターンでも・獲得先がどこでも）。
       別の獲得時効果で先に動かされていたら使用に失敗する（stop-moving）＝キュー消化時に獲得先を再確認する。 */
    if (cardId === 'buried_treasure') {
      (state.onGainQueue = state.onGainQueue || []).push({ type: 'buried_treasure_play', player: pIndex, dest });
    }
    /* セイレーン＝これを獲得したとき、手札からアクション1枚を廃棄してもよい。廃棄しないならセイレーンを廃棄する。
       獲得先から先に動かされていたら自己廃棄は失敗する（それでもアクションの廃棄は選べる＝公式）。 */
    if (cardId === 'siren') {
      (state.onGainQueue = state.onGainQueue || []).push({ type: 'siren_gain', player: pIndex, dest });
    }
    ...
    /* 密航者＝**誰かが**持続カード1枚を獲得したとき、手札から使用してもよい（移動動物園型リアクション）。
       ...
       手札に獲得した密航者自身も、その獲得にリアクションできる（＝消化時に手札を見る）。 */
    if (DOM.isType(cardId, 'duration')) {
      for (let k = 0; k < n; k++) {
        const seat = ((state.turn ? state.turn.active : pIndex) + k) % n;
        if (state.players[seat].hand.includes('stowaway') || (seat === pIndex && dest === 'hand' && cardId === 'stowaway')) {
          (state.onGainQueue = state.onGainQueue || []).push({ type: 'stowaway_react', player: seat });
        }
      }
    }
    ...
    /* 同盟：道化棒（bauble）の4つ目の選択肢も**まったく同じ窓**（`This turn, when you gain a card,
       you may put it onto your deck.`）＝同じ pending を共有する（`source` で表示だけ分ける）。
       "this turn," なので道化棒が場を離れても効き続ける（偽造通貨で廃棄しても有効＝公式）。 */
    if (state.turn && (state.turn.travellingFair || state.turn.baubleTopdeck) && pIndex === state.turn.active && dest !== 'deck') {
      (state.onGainQueue = state.onGainQueue || []).push({ type: 'travelling_fair', player: pIndex, card: cardId,
        dest: dest || 'discard', source: state.turn.travellingFair ? 'travelling_fair' : 'bauble' });
    }
    /* ===== 同盟 A4：王国カードの「カードを獲得したとき」＝**Ally の窓より前**に解決する =====
       ⚠ ギルドマスターの +1好意 は Ally の窓（建築家ギルド/都市国家/遊牧民団/罠師の小屋）より**先**に入れる
         （公式FAQ逐語＝`If an Ally ability triggers on gaining cards, e.g. Band of Nomads,
          you can use the Favor you just got on it.`）。逆順にすると「今もらった好意をその場で使う」が再現できない。 */
    {
      const myTurn = !!(state.turn && state.turn.active === pIndex);
      // ごますり＝**獲得または廃棄したとき** +2好意（カード自身の能力＝どのゾーンから獲得しても働く）。
      if (cardId === 'sycophant') gainFavors(state, pIndex, 2, 'はごますりの獲得で');
      /* ギルドマスター＝「**このターン**、カードを1枚獲得するとき +1好意」。
         "This turn," なので**場を離れても効き続け、使用回数ぶん累積する**（"while this is in play" ではない）。 */
      if (myTurn && (state.turn.guildmasters || 0) > 0) {
        gainFavors(state, pIndex, state.turn.guildmasters, 'はギルドマスターで');
      }
      /* ガレリア＝「このターン、**ちょうど$3か$4**のカードを獲得するとき +1購入」。使用回数ぶん累積。
         ⚠ コストの**読み取り**なので `costExact`（＝gainableBase 内包＝最後の1枚を取ると supply が0で false）
            ではなく `costOf` を直接使う。非サプライ札（馬=$3）でも公式どおり出る。 */
      if (myTurn && (state.turn.galleria || 0) > 0) {
        const gc = costOf(state, cardId);
        if (gc.pot === 0 && gc.debt === 0 && (gc.coin === 3 || gc.coin === 4)) {
          state.turn.buys += state.turn.galleria;
          log(state, `${state.players[pIndex].name} はガレリアで +${state.turn.galleria}購入（コスト$${gc.coin} のカードを獲得）。`);
        }
      }
      /* 駐屯地＝場に出ている駐屯地の**プレイ回数ぶん**、その物理カードの上にトークンを1個ずつ置く。
         **好意ではない**（自前のカウンタ）。玉座で2回使うと1獲得につき2個（＝除去は1回なので合計6個で+6カード）。 */
      if (myTurn && (state.turn.garrisonTokens || []).length) {
        for (let gi = 0; gi < state.turn.garrisonTokens.length; gi++) state.turn.garrisonTokens[gi] += 1;
      }
      /* 散兵＝「このターン、**アタックカード**を獲得するとき、他の全員が手札3枚になるように捨てる」。
         **使用回数ぶん独立に発動**（N枚使えばN回。各回が独立に「3枚まで」を再実行する＝公式FAQ逐語）。
         免疫は**使用した瞬間**に確定済み（獲得時に堀は公開できない＝公式）。
         ⚠ `state.pending` 直代入をせず onGainQueue に積む（望楼/そり等の窓を握りつぶさない）。 */
      if (myTurn && (state.turn.skirmishers || []).length && DOM.isType(cardId, 'attack')) {
        (state.turn.skirmishers || []).forEach((sk) => {
          (state.onGainQueue = state.onGainQueue || []).push({ type: 'skirmisher_attack', player: pIndex, immune: (sk.immune || []).slice() });
        });
      }
      /* 領土＝獲得したとき、**サプライの空の山1つにつき金貨1枚**。
         公式FAQ逐語＝`If gaining Territory causes the Clashes pile to be empty, that counts`
         ＝**この獲得で空になった山も数える**（＝獲得を済ませたこの位置で数える）。 */
      if (cardId === 'territory') {
        const n = emptyPileCount(state);
        let g = 0;
        for (let i = 0; i < n; i++) if (gain(state, pIndex, 'gold', 'discard')) g++;
        if (n > 0) log(state, `${state.players[pIndex].name} は領土の獲得で金貨${g}枚を獲得した（空の山${n}個）。`);
      }
    }
  ...
  function triggerOnTrash(state, pIndex, card, opts) {
    ...
    /* 略奪P3：宝飾卵＝これを廃棄したとき、戦利品1枚を獲得する（**廃棄した本人**＝誰のカードの効果で
       ...
       宝飾卵の on-trash は所有条件が無いので fromSupply でも発動してよい＝塩まき等では実際には起きない）。 */
    if (card === 'jewelled_egg') {
      log(state, `${p.name} は宝飾卵の廃棄で戦利品を獲得する。`);
      gainLoot(state, pIndex);
    }
    ...
    /* 同盟：ごますり＝**獲得または廃棄したとき** +2好意。**サプライからの廃棄（待ち伏せ等）でも発動する**
  ...
  function cleanupAndAdvance(state) {
    ...
    /* 同盟：駐屯地＝トークンが1個以上乗っていれば**そのときだけ**持続の予約を張る（条件つき持続）。
       ⚠ 「場を離れたらトークンは無い」（公式）＝場に駐屯地が1枚も無ければ全部捨てる。
       ⚠ 玉座の間で1枚を2回使うと予約は2件（各3個＝合計6個で +6カード）だが**物理カードは1枚**＝
          場の枚数を超える予約はまとめる（除去は1回だけ＝公式FAQ「12枚ではなく6枚」）。 */
    /* 略奪P3：現場監督＝このターン、プレイ後にコストちょうど$5のカードを獲得していた予約ぶん、
       次のターンの開始時に能力を繰り返す（＝片付けで予約を張って持続として場に残す。駐屯地と同じ形）。 */
    (state.turn.taskmasterWatch || []).forEach((w) => { if (w.hit) armDuration(state, w.player, 'taskmaster'); });
    state.turn.taskmasterWatch = null;
    {
      const gt = (state.turn.garrisonTokens || []).filter((n) => n > 0);
      const physical = p.inPlay.filter((c) => c === 'garrison').length;
      if (gt.length && physical > 0) {
        const buckets = new Array(Math.min(gt.length, physical)).fill(0);
        gt.forEach((n, i) => { buckets[i % buckets.length] += n; });
  ...
  function endBuyTailSchemeOrCleanup(state, pi) {
    ...
    /* 同盟：天幕／商人の野営地＝「**あなたがこのカードを場から捨て札にするとき**、山札の一番上に置いてよい」。
       ...
       - **本エンジンは片付けで次の手札を先引きする**ので、置く処理は**必ず先引きより前**（＝この位置。
         策謀／角笛／宝物庫と同じスロット）。後にすると1ターン遅れてほぼ無効化される。
       - 記載効果を置き換える効果（習性・女魔術師）を受けても働く（カード自身の静的能力＝公式）。
       - 「場に出さずに使用」（命令/ネクロマンサー/相続/王子/ハツカネズミの習性）や、場を離れた場合は
         そもそも場に無いので候補に出ない＝幻のカードが山札に増えない（保存則）。 */
    if (!state.turn.topdeckDone) {
      const cand = me.inPlay.filter((c) => TOPDECK_ON_DISCARD.has(c));
      if (cand.length) {
        state.turn.topdeckDone = true;
        state.pending = { type: 'allies_topdeck', player: pi, cards: cand };
        return;
      }
      state.turn.topdeckDone = true;
    }
  ...
  function reduce(state, action) {
    ...
    if (!state.pending && !state.gameOver && state.turn && state.turn.hvPending) {
      const hp = state.turn.hvPending;
      state.turn.hvPending = null;
    ...
    if (!state.pending && !state.gameOver && state.onGainQueue && state.onGainQueue.length) {
      ...
      while (state.onGainQueue.length) {
        ...
        if (q.type === 'buried_treasure_play') {
          const bp = state.players[q.player];
          const bz = zoneOf(bp, q.dest);
          if (bz && bz.indexOf('buried_treasure') >= 0) {
            playCardNoAction(state, q.player, 'buried_treasure', bz, '獲得時効果で');
          } else {
            log(state, `${bp.name} の埋められた財宝は使用に失敗した（獲得先から動かされていた）。`);
          }
        ...
        if (q.type === 'siren_gain' && !state.players[q.player].hand.some((c) => DOM.isType(c, 'action'))) {
          ...
          const sz = zoneOf(sp, q.dest);
          if (sz && removeOne(sz, 'siren')) {
            trashCard(state, q.player, 'siren');
            log(state, `${sp.name} はセイレーンを廃棄した（手札にアクションが無い）。`);
          }
          continue;
        }
  ...
  function runReplays(state) {
    // 同盟：長老の追加選択は**再演では付かない**（公式FAQ逐語）＝ここでブーストを落とす。
    if (state.turn && (state.replay || []).length) state.turn.elderBoost = null;
    let guard = 0;
  ...
  function applyAction(state, action) {
    ...
    switch (action.type) {
      ...
      case 'PLAY_ACTION': {
        ...
        /* 同盟：長老の追加選択は「長老が使わせた**その1枚**」だけに効く（公式）。
           自分でアクションを使い始めたらブーストを落とす（同名の2枚目に付かない／再演にも付かない）。 */
        t.elderBoost = null;
      ...
      /* ===== 略奪P3 ===== */
      // 岩屋＝手札を最大4枚 伏せて脇（setAside）へ。0枚なら持続にならない（公式）。
      case 'GROTTO_SET': {
        const pd = state.pending;
        if (!pd || pd.type !== 'grotto_set') return state;
        const pl = state.players[pd.player];
        const cards = Array.isArray(action.cards) ? action.cards.slice(0, 4) : [];
        const chk = pl.hand.slice();
        for (const c of cards) { const i = chk.indexOf(c); if (i < 0) return state; chk.splice(i, 1); }
        state.pending = null;
        if (cards.length) {
          cards.forEach((c) => { removeOne(pl.hand, c); pl.setAside.push(c); });
          armDuration(state, pd.player, 'grotto', { stashed: cards.slice() });
          log(state, `${pl.name} は岩屋に ${cards.length}枚 を伏せて置いた（次のターンの開始時に捨てて同じ枚数を引く）。`);
        }
        return state;
      }
      // シャーマン＝手札1枚を廃棄してもよい（任意）。
      case 'SHAMAN_TRASH': {
        const pd = state.pending;
        if (!pd || pd.type !== 'shaman_trash') return state;
        const pl = state.players[pd.player];
        if (action.card == null) { state.pending = null; return state; }
        if (!removeOne(pl.hand, action.card)) return state;
        state.pending = null;
        trashCard(state, pd.player, action.card);
        log(state, `${pl.name} はシャーマンで「${C()[action.card].name}」を廃棄した。`);
        return state;
      }
      /* シャーマン（常設）＝廃棄置き場からコスト6以下のカード1枚を獲得（**強制**・候補があれば必ず）。
         候補は shamanTargets が正本（engine/CPU/UI 共通）。候補が枯れていたら null で閉じる（終端保証）。 */
      case 'SHAMAN_GAIN': {
        const pd = state.pending;
        if (!pd || pd.type !== 'shaman_gain') return state;
        const targets = shamanTargets(state);
        if (action.card == null) {
          if (targets.length) return state; // 候補がある間は獲得が強制
          state.pending = null; return state;
        }
        if (targets.indexOf(action.card) < 0) return state;
        const ti = state.trash.indexOf(action.card);
        if (ti < 0) return state;
        state.pending = null;
        state.trash.splice(ti, 1);
        log(state, `${state.players[pd.player].name} はシャーマンの効果で廃棄置き場から「${C()[action.card].name}」を獲得した。`);
        gainFromOutside(state, pd.player, action.card, 'discard');
        return state;
      }
      // セイレーン＝アタックのリアクション（堀を出さない＝呪いを受けて次の被害者へ）。
      case 'SIREN_REACT': {
        const pd = state.pending;
        if (!pd || pd.type !== 'siren' || pd.stage !== 'react') return state;
      ...
      case 'SIREN_GAIN': {
        const pd = state.pending;
        if (!pd || pd.type !== 'siren_gain') return state;
        const pl = state.players[pd.player];
        if (action.card != null) {
          if (!DOM.isType(action.card, 'action')) return state;
          if (!removeOne(pl.hand, action.card)) return state;
          state.pending = null;
          trashCard(state, pd.player, action.card);
          log(state, `${pl.name} は「${C()[action.card].name}」を廃棄してセイレーンを守った。`);
        } else {
          state.pending = null;
          const z = zoneOf(pl, pd.dest);
          if (z && removeOne(z, 'siren')) {
            trashCard(state, pd.player, 'siren');
            log(state, `${pl.name} はセイレーンを廃棄した。`);
          } else {
            log(state, `${pl.name} のセイレーンは廃棄に失敗した（獲得先から動かされていた）。`);
          }
        }
        return state;
      }
      /* 密航者＝誰かの持続獲得に反応して手札から使用してもよい（牧羊犬と同型＝場に出して効果を適用）。
         使用後も手札に密航者が残っていれば、同じ獲得にもう1枚 反応できる（公式）。 */
      case 'STOWAWAY_REACT': {
        const pd = state.pending;
        if (!pd || pd.type !== 'stowaway_react') return state;
        state.pending = null;
        if (!action.play) return state;
        const pl = state.players[pd.player];
        if (!removeOne(pl.hand, 'stowaway')) return state;
        pl.inPlay.push('stowaway');
        log(state, `${pl.name} はリアクションで「密航者」を使った（次の自分のターンの開始時 +2カード）。`);
        noteAllyPlay(state, pd.player, 'stowaway'); // これも「カードの使用」（旗艦/上陸部隊/Ally が正しく絡む）
        applyEffect(state, 'stowaway', pd.player);
        if (pl.hand.indexOf('stowaway') >= 0) {
          (state.onGainQueue = state.onGainQueue || []).push({ type: 'stowaway_react', player: pd.player });
        }
        return state;
      }
      // 置き去り＝手札1枚を廃棄（強制）→ その**静的な**種別1つにつき +2カード。
      case 'MAROON_TRASH': {
        const pd = state.pending;
        if (!pd || pd.type !== 'maroon_trash') return state;
        const pl = state.players[pd.player];
        if (action.card == null || !removeOne(pl.hand, action.card)) return state;
        state.pending = null;
        trashCard(state, pd.player, action.card);
        const types = (C()[action.card].types || []).length;
        draw(state, pd.player, 2 * types);
        log(state, `${pl.name} は置き去りで「${C()[action.card].name}」を廃棄し +${2 * types}カード（種別${types}つ）。`);
        return state;
      }
      // 坩堝＝手札1枚を廃棄（強制）→ 廃棄後のコストのコイン成分ぶん +$。
      case 'CRUCIBLE_TRASH': {
        const pd = state.pending;
        if (!pd || pd.type !== 'crucible_trash') return state;
        const pl = state.players[pd.player];
        if (action.card == null || !removeOne(pl.hand, action.card)) return state;
        state.pending = null;
        trashCard(state, pd.player, action.card);
        const coin = cardCost(state, action.card);
        if (coin > 0) addCoins(state, coin);
        log(state, `${pl.name} は坩堝で「${C()[action.card].name}」を廃棄し +$${coin}。`);
        return state;
      }
      // 巡礼者＝手札1枚を山札の上に置く（強制・候補は手札全部）。
      case 'PILGRIM_PUT': {
        const pd = state.pending;
        if (!pd || pd.type !== 'pilgrim_put') return state;
        const pl = state.players[pd.player];
        if (action.card == null || !removeOne(pl.hand, action.card)) return state;
        state.pending = null;
        pl.deck.unshift(action.card);
        log(state, `${pl.name} は巡礼者で手札1枚を山札の上に置いた。`);
        return state;
      }
      // 小像＝手札のアクション1枚を捨ててもよい→捨てたら +1購入 +$1（捨て札トリガーを通す）。
      case 'FIGURINE_DISCARD': {
        const pd = state.pending;
        if (!pd || pd.type !== 'figurine_discard') return state;
        const pl = state.players[pd.player];
        if (action.card == null) { state.pending = null; return state; }
        if (!DOM.isType(action.card, 'action')) return state;
        if (!removeOne(pl.hand, action.card)) return state;
        ...
        log(state, `${pl.name} は小像で「${C()[action.card].name}」を捨て、+1購入 +$1。`);
        triggerOnDiscard(state, pd.player, [action.card]);
        t.buys += 1; addCoins(state, 1);
        return state;
      }
      // ゴンドラ＝「今 +$2」か「次の自分のターンの開始時 +$2」。
      case 'GONDOLA_CHOOSE': {
        const pd = state.pending;
        if (!pd || pd.type !== 'gondola_choose') return state;
        state.pending = null;
        if (action.now) { addCoins(state, 2); log(state, `${state.players[pd.player].name} はゴンドラで今 +$2 を得た。`); }
        else { armDuration(state, pd.player, 'gondola'); log(state, `${state.players[pd.player].name} はゴンドラの +$2 を次のターンの開始時に受ける。`); }
        return state;
      }
      // ゴンドラの獲得時＝手札のアクション1枚を使用してもよい（アクション権は消費しない）。
      case 'GONDOLA_PLAY': {
        const pd = state.pending;
      ...
      /* 工具＝誰かが場に出しているのと同じカード1枚を獲得（強制）。サプライに無い札（戦利品/闇市場由来）も
         **選べるが獲得は起きない**（公式＝探索/専門家と同じ「遂行できない選択肢も選べる」）。 */
      case 'TOOLS_GAIN': {
        const pd = state.pending;
        if (!pd || pd.type !== 'tools_gain') return state;
        if (action.card == null || toolsTargets(state).indexOf(action.card) < 0) return state;
        state.pending = null;
        const key = gainableBase(state, action.card) ? action.card : mixedPileWithTop(state, action.card);
        if (key && gain(state, pd.player, key, 'discard')) {
          log(state, `${state.players[pd.player].name} は工具で「${C()[action.card].name}」を獲得した。`);
        } else {
          log(state, `${state.players[pd.player].name} は工具で「${C()[action.card].name}」を選んだが、サプライに無いため獲得できない。`);
        }
        return state;
      }
      // つるはし＝手札1枚を廃棄（強制）→ 廃棄**後**のコストが3以上なら戦利品1枚を**手札に**獲得。
      case 'PICKAXE_TRASH': {
        const pd = state.pending;
        if (!pd || pd.type !== 'pickaxe_trash') return state;
        const pl = state.players[pd.player];
        if (action.card == null || !removeOne(pl.hand, action.card)) return state;
        state.pending = null;
        trashCard(state, pd.player, action.card);
        const coin = cardCost(state, action.card); // コスト参照は廃棄の「後」（必読J）
        if (coin >= 3) {
          log(state, `${pl.name} はつるはしで「${C()[action.card].name}」（コスト${coin}）を廃棄し、戦利品を手札に獲得する。`);
          gainLoot(state, pd.player, 'hand');
        } else {
          log(state, `${pl.name} はつるはしで「${C()[action.card].name}」（コスト${coin}）を廃棄した。`);
        }
        return state;
      }
      // 銀山＝これより安い財宝1枚を**手札に**獲得（強制）。
      case 'SILVER_MINE_GAIN': {
        const pd = state.pending;
        if (!pd || pd.type !== 'silver_mine_gain') return state;
        const ref = costOf(state, 'silver_mine');
        const okId = (id) => gainableBase(state, id) && isTypeSupply(state, id, 'treasure') && costLT(costOf(state, id), ref);
        if (action.card == null) {
          if (anyGainable(state, okId)) return state; // 候補がある間は獲得が強制
          state.pending = null; return state;
        }
        if (!okId(action.card)) return state;
        state.pending = null;
        if (gain(state, pd.player, action.card, 'hand')) {
          log(state, `${state.players[pd.player].name} は銀山で「${C()[action.card].name}」を手札に獲得した。`);
        }
        return state;
      }
      /* キャビンボーイ＝次のターンの開始時の二択。「廃棄して持続を獲得」は**場から廃棄できたときだけ**獲得が起きる
         （必読14c＝takeSelf 失敗なら獲得も無し。持続の獲得に**コスト制限は無い**＝costUpTo を掛けない）。 */
      case 'CABIN_BOY_RESOLVE': {
        const pd = state.pending;
        if (!pd || pd.type !== 'cabin_boy') return state;
        const pl = state.players[pd.player];
        state.pending = null;
        if (action.choice === 'gain') {
          if (removeOne(pl.durationCards || [], 'cabin_boy') || removeOne(pl.inPlay, 'cabin_boy')) {
            trashCard(state, pd.player, 'cabin_boy');
            log(state, `${pl.name} はキャビンボーイを廃棄した（持続カードを獲得する）。`);
            if (anyGainable(state, (id) => gainableBase(state, id) && isTypeSupply(state, id, 'duration'))) {
              state.pending = { type: 'cabin_boy_gain', player: pd.player };
            } else {
              log(state, `${pl.name} は獲得できる持続カードがサプライに無い。`);
            }
          } else {
            log(state, `${pl.name} のキャビンボーイは場を離れていて廃棄できない（獲得も起きない）。`);
          }
        } else {
          addCoins(state, 2);
          log(state, `${pl.name} はキャビンボーイで +$2。`);
        }
        return state;
      }
      case 'CABIN_BOY_GAIN': {
        const pd = state.pending;
        if (!pd || pd.type !== 'cabin_boy_gain') return state;
        const okId = (id) => gainableBase(state, id) && isTypeSupply(state, id, 'duration');
        if (action.card == null) {
        ...
        if (gain(state, pd.player, action.card, 'discard')) {
          log(state, `${state.players[pd.player].name} はキャビンボーイの効果で「${C()[action.card].name}」を獲得した。`);
        }
        return state;
      }
      // 縄＝次のターンの開始時、+1カードの後に手札1枚を廃棄してもよい（任意）。
      case 'ROPE_TRASH': {
        const pd = state.pending;
        if (!pd || pd.type !== 'rope_trash') return state;
        const pl = state.players[pd.player];
        if (action.card == null) { state.pending = null; return state; }
        if (!removeOne(pl.hand, action.card)) return state;
        state.pending = null;
      ...
      case 'ALLY_SIMPLE': {
        ...
        if (pd.type === 'ally_city_state') {
          ...
          /* ⚠ 同盟：**手札に直接獲得した**カードを使う場合だけ航海の3枚制限に掛かる（公式FAQ逐語）＝
             使えないなら好意を払わせない。獲得先が捨て札/山札なら「手札からのプレイ」ではないので通る。 */
          if (pd.dest === 'hand' && !canPlayHandCard(state, pd.player, pd.card)) { state.pending = null; return state; }
      ...
      case 'ALLY_MARKET_TOWNS': {
        ...
        /* ⚠ 同盟：航海の3枚制限／将軍は「手札から使わせる能力」も上書きする（公式逐語＝
           `Voyage's restriction will override any ability that lets you play cards from your hand
           (such as Market Towns...)`）＝**使えないなら好意を払わせない**（払い損を作らない）。 */
      ...
      /* 同盟：循環(Rotate)＝**常に任意**。「一番上のカードと、その直下に連続する同名のカード」だけを山の一番下へ。
         - `pd.pile` が入っていれば自分の山を名指しした窓（薬草集め/古地図/天幕/触れ役/生徒）＝Yes/No だけ。
         - `pd.any` なら戦闘計画＝**任意のサプライ山**（`rotatableSupplyPiles` が正本。回しても何も起きない山も選べる）。
         - `pd.next` があれば解決後にそこへ進む（生徒＝循環が先で、その後に強制廃棄）。 */
      case 'ROTATE_PILE': {
        const pd = state.pending;
        if (!pd || pd.type !== 'rotate_pile') return state;
        const pick = action.pile;
        if (pick != null) {
          const legal = pd.any ? rotatableSupplyPiles(state).indexOf(pick) >= 0 : pick === pd.pile;
          if (!legal) return state;
          const moved = rotatePile(state, pick);
          const nm = (C()[pick] || {}).name || pick;
          log(state, `${state.players[pd.player].name} は「${nm}」の山を循環させた${moved ? '' : '（順序は変わらなかった）'}。`);
        }
        state.pending = pd.next || null;
        return state;
      }
      /* 天幕／商人の野営地＝場から捨てるとき、好きな枚数を山札の上に置く（任意・0枚可）。
         置いた札は片付けの「場を捨てる」から外れる（＝先引きでそのまま次の手札に入り得る＝公式）。 */
      case 'ALLIES_TOPDECK': {
        const pd = state.pending;
        if (!pd || pd.type !== 'allies_topdeck') return state;
        const owner = state.players[pd.player];
        const cards = Array.isArray(action.cards) ? action.cards : [];
        const copy = pd.cards.slice();
        for (const c of cards) if (!removeOne(copy, c)) return state; // 場にある候補の部分集合のみ
        cards.forEach((c) => { if (removeOne(owner.inPlay, c)) owner.deck.unshift(c); });
        if (cards.length) log(state, `${owner.name} は ${cards.length}枚 を山札の一番上に置いた（場から捨てる代わり）。`);
        state.pending = null;
        endBuyTailSchemeOrCleanup(state, pd.player);
        return state;
      }
      // 沈没船の財宝＝場に同名が無いアクション1枚を獲得（強制・コスト制限なし）。
      case 'SUNKEN_TREASURE_GAIN': {
        const pd = state.pending;
      ...
      case 'SENTINEL_TRASH': {
        ...
        // ⚠ **同時に廃棄する**（全部を廃棄置き場に入れてから on-trash＝リッチが同時廃棄の札を獲得できる＝公式例）。
        //    廃棄でドローする札（ネズミ/狂信者）は「今の山札」から引く（見ている5枚は既に deck から抜いてある）。
        trashCardsTogether(state, pd.player, tr);
      ...
      case 'MODIFY_CHOOSE': {
        ...
        if (!opts) return state;
        state.pending = null;
        runChoiceOptions(state, 'modify', pd.player, opts, { coin: pd.coin, pot: pd.pot, debt: pd.debt, noTrash: !!pd.noTrash });
        return state;
      }
      case 'MODIFY_GAIN': {
        const pd = state.pending;
      ...
      case 'BAUBLE_CHOOSE': {
        const pd = state.pending;
        if (!pd || pd.type !== 'bauble_choose') return state;
        const picks = Array.isArray(action.picks) ? action.picks : [];
        const valid = ['buy', 'coin', 'favor', 'topdeck'];
        if (picks.length !== 2 || picks[0] === picks[1]) return state;
        if (picks.some((c) => valid.indexOf(c) < 0)) return state;
        state.pending = null;
        const owner = state.players[pd.player];
        valid.forEach((opt) => {                    // 記載順に解決（+1購入 → +$1 → +1好意 → 山札の上に置く窓）
          if (picks.indexOf(opt) < 0) return;
          if (opt === 'buy') t.buys += 1;
          else if (opt === 'coin') addCoins(state, 1);
          else if (opt === 'favor') gainFavors(state, pd.player, 1, 'は道化棒で');
          else t.baubleTopdeck = (t.baubleTopdeck || 0) + 1;
        });
        log(state, `${owner.name} は道化棒で ${picks.length}つ を選んだ。`);
        return state;
      }
      /* 契約書＝手札のアクション1枚を脇に置いてよい（任意）。置いたときだけ持続になる。 */
      case 'CONTRACT_SETASIDE': {
        const pd = state.pending;
        if (!pd || pd.type !== 'contract_setaside') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        if (card == null) { state.pending = null; return state; }   // 辞退＝持続にならない
        if (owner.hand.indexOf(card) < 0) return state;
        if (!DOM.isType(card, 'action') && !inheritedEstate(owner, card)) return state;
        removeOne(owner.hand, card);
        (owner.contractSetAside = owner.contractSetAside || []).push(card);
        armDuration(state, pd.player, 'contract');
        log(state, `${owner.name} は契約書で「${C()[card].name}」を脇に置いた（次のターンの開始時に使用）。`);
        state.pending = null;
        return state;
      }
      // 契約書＝次のターンの開始時に脇札を使用する（強制・アクション権を消費しない）。
      case 'CONTRACT_PLAY': {
        const pd = state.pending;
        if (!pd || pd.type !== 'contract_play') return state;
        const owner = state.players[pd.player];
        const z = owner.contractSetAside || [];
        if (!z.length) { state.pending = null; return state; }
        const card = z[0];
        state.pending = null;
        playCardNoAction(state, pd.player, card, z, '契約書で脇に置いた', action.way);
        return state;
      }
      // 輸入者＝次のターンの開始時に $5以下のカード1枚を獲得（強制）。
      case 'IMPORTER_GAIN': {
        const pd = state.pending;
        if (!pd || pd.type !== 'importer_gain') return state;
        const card = action.card;
      ...
      /* 仲買人＝手札1枚を廃棄（強制）→ 4択（そのコイン費用$1につき1つ）。
         ⚠ 廃棄したカードの**コイン費用**だけを見る（負債・ポーション成分は0扱い＝公式FAQ逐語）。 */
      case 'BROKER_TRASH': {
        const pd = state.pending;
        if (!pd || pd.type !== 'broker_trash') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        if (card == null || owner.hand.indexOf(card) < 0) return state; // 強制
        removeOne(owner.hand, card);
      ...
      /* 生徒＝手札1枚を廃棄（強制）。財宝なら +1好意 かつ**これ（場の生徒）を山札の上に置く**。
         ⚠ 「これ」が場に無い（命令経由など）ときは好意だけ得て移動は不発（lose track＝保存則）。 */
      case 'STUDENT_TRASH': {
        const pd = state.pending;
        if (!pd || pd.type !== 'student_trash') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        if (card == null || owner.hand.indexOf(card) < 0) return state; // 強制
        ...
        removeOne(owner.hand, card);
        trashCard(state, pd.player, card);
        log(state, `${owner.name} は生徒で「${C()[card].name}」を廃棄した。`);
        if (wasTreasure) {
          gainFavors(state, pd.player, 1, 'は生徒で');
          if (takeSelf(state, pd.player, 'student')) {
            owner.deck.unshift('student');
            log(state, `${owner.name} は生徒を山札の一番上に置いた。`);
          }
        }
        if (state.pending === pd) state.pending = null;
        return state;
      }
      // 触れ役＝三択 → その後に町民の山を循環してよい（何を選んでも循環は選べる＝公式）。
      ...
      /* 薬草集め＝捨て札から財宝1枚を使用してよい（任意・アクション権を消費しない）→ その後に卜占官を循環してよい。 */
      case 'HERB_GATHERER_PLAY': {
        const pd = state.pending;
        if (!pd || pd.type !== 'herb_gatherer_play') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        state.pending = null;
        if (card != null) {
          if (owner.discard.indexOf(card) < 0 || !isTreasureFor(state, card)) return state;
          playCardNoAction(state, pd.player, card, owner.discard, '薬草集めで捨て札から');
        }
        const rot = { type: 'rotate_pile', player: pd.player, pile: 'augurs', source: 'herb_gatherer', next: null };
        if (state.pending) (state.onGainQueue = state.onGainQueue || []).push(rot);
        else if (canRotatePile(state, 'augurs')) state.pending = rot;
        return state;
      }
      // 古地図＝手札1枚を捨てる（強制）→ +1カード → 叙事詩を循環してよい。
      case 'OLD_MAP_DISCARD': {
        const pd = state.pending;
        if (!pd || pd.type !== 'old_map_discard') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        if (card == null || owner.hand.indexOf(card) < 0) return state; // 強制
        state.pending = null;
        alliesDiscardHand(state, pd.player, [card], '古地図で');
        draw(state, pd.player, 1);
        const rot = { type: 'rotate_pile', player: pd.player, pile: 'odysseys', source: 'old_map', next: null };
        if (state.pending) (state.onGainQueue = state.onGainQueue || []).push(rot);
        else if (canRotatePile(state, 'odysseys')) state.pending = rot;
        return state;
      }
      // 戦闘計画＝手札のアタック1枚を公開してよい（+1カード）→ 任意のサプライ山を循環してよい。
      case 'BATTLE_PLAN_REVEAL': {
        const pd = state.pending;
        if (!pd || pd.type !== 'battle_plan_reveal') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        if (card != null) {
          if (owner.hand.indexOf(card) < 0 || !DOM.isType(card, 'attack')) return state;
          reveal(state, pd.player, [card], '戦闘計画でアタックカードを公開');
          draw(state, pd.player, 1);
          log(state, `${owner.name} は戦闘計画で「${C()[card].name}」を公開して +1カード。`);
        }
        state.pending = { type: 'rotate_pile', player: pd.player, any: true, source: 'battle_plan' };
        return state;
      }
      // 町＝二択（+1カード+2アクション ／ +1購入+2コイン）。強制。
      case 'TOWN_CHOOSE': {
      ...
      /* 蹄鉄工＝三択（手札6枚になるまで引く ／ +2カード ／ +1カード+1アクション）。強制。
         「6枚になるまで」は既に6枚以上なら1枚も引かない（`draw until` の一般規約＝図書館と同型）。 */
      ...
      case 'MILLER_PICK': {
        const pd = state.pending;
        if (!pd || pd.type !== 'miller_pick') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        if (card == null || pd.cards.indexOf(card) < 0) return state; // 強制＝辞退できない
        const rest = pd.cards.slice();
        removeOne(rest, card);
        owner.hand.push(card);
        log(state, `${owner.name} は粉屋で「${C()[card].name}」を手札に加えた。`);
        state.pending = null;
        if (rest.length) { rest.forEach((c) => owner.discard.push(c)); triggerOnDiscard(state, pd.player, rest); }
        return state;
      }
      // 侯爵＝手札が10枚になるように捨てる（枚数は強制・中身は任意）。
      case 'MARQUIS_DISCARD': {
        const pd = state.pending;
        if (!pd || pd.type !== 'marquis_discard') return state;
        const owner = state.players[pd.player];
        const need = Math.max(0, owner.hand.length - 10);
        const cards = Array.isArray(action.cards) ? action.cards : [];
        if (cards.length !== need) return state;
        const copy = owner.hand.slice();
        for (const c of cards) if (!removeOne(copy, c)) return state;
        state.pending = null;
        alliesDiscardHand(state, pd.player, cards, '侯爵で');
        return state;
      }
      // ごますり＝手札3枚を**同時に**捨てる（強制）。1枚以上捨てたら +3コイン。
      case 'SYCOPHANT_DISCARD': {
        const pd = state.pending;
        if (!pd || pd.type !== 'sycophant_discard') return state;
        const owner = state.players[pd.player];
        const need = Math.min(3, owner.hand.length);
        const cards = Array.isArray(action.cards) ? action.cards : [];
        if (cards.length !== need) return state;
        const copy = owner.hand.slice();
        for (const c of cards) if (!removeOne(copy, c)) return state;
        state.pending = null;
        sycophantDiscard(state, pd.player, cards);
        return state;
      }
      /* 女予言者＝手札1枚を山札の上に、もう1枚を山札の一番下に（強制・この順）。
         公式FAQ逐語＝`If after drawing your deck has no cards in it, the first card you put back
         will become the top card of it.` ＝**上→下の順で置く**（同時に振り分ける実装では再現できない）。 */
      case 'SIBYL_PLACE': {
        const pd = state.pending;
        if (!pd || pd.type !== 'sibyl_place') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        if (card == null || owner.hand.indexOf(card) < 0) return state; // 強制
        removeOne(owner.hand, card);
        if (pd.stage === 'top') {
          ...
          log(state, `${owner.name} は女予言者で手札1枚を山札の上に置いた。`);
          state.pending = owner.hand.length > 0 ? { type: 'sibyl_place', stage: 'bottom', player: pd.player } : null;
        } else {
          owner.deck.push(card);
          log(state, `${owner.name} は女予言者で手札1枚を山札の一番下に置いた。`);
          state.pending = null;
        }
        return state;
      }
      /* 首都＝①手札2枚を捨てて +2コイン（任意・**2枚ちょうど捨てたときだけ**コインが出る。
         手札が2枚未満でも「捨てる」を選べる＝公式FAQ）→ ②2コインを払って +2カード（任意）。この順。 */
      case 'CAPITAL_CITY': {
        const pd = state.pending;
        if (!pd || pd.type !== 'capital_city') return state;
        const owner = state.players[pd.player];
        if (pd.stage === 'discard') {
          if (action.ok) {
            const cards = Array.isArray(action.cards) ? action.cards : [];
            ...
            const copy = owner.hand.slice();
            for (const c of cards) if (!removeOne(copy, c)) return state;
            const moved = alliesDiscardHand(state, pd.player, cards, '首都で');
            if (moved.length === 2) { addCoins(state, 2); log(state, `${owner.name} は首都で +2コイン。`); }
          }
          state.pending = (t.coins || 0) >= 2 ? { type: 'capital_city', stage: 'pay', player: pd.player } : null;
          return state;
        }
        // stage 'pay'：2コインを払って +2カード（**購入ではない支払い**＝購入権や treasuresLocked に触れない）。
        if (action.ok) {
          if ((t.coins || 0) < 2) return state;
          t.coins -= 2;
          draw(state, pd.player, 2);
          log(state, `${owner.name} は首都で 2コインを払って +2カード。`);
        }
        state.pending = null;
        return state;
      }
      /* 宿屋の主人＝三択（+1カード ／ +3カード後に3枚捨て ／ +5カード後に6枚捨て）。**引く前に選ぶ**（強制）。
         捨てる枚数は強制（手札が足りなければあるだけ）。 */
      ...
      case 'INNKEEPER_DISCARD': {
        const pd = state.pending;
        if (!pd || pd.type !== 'innkeeper_discard') return state;
        const owner = state.players[pd.player];
        const need = Math.min(pd.n, owner.hand.length);
        const cards = Array.isArray(action.cards) ? action.cards : [];
        if (cards.length !== need) return state;
        const copy = owner.hand.slice();
        for (const c of cards) if (!removeOne(copy, c)) return state;
        state.pending = null;
        alliesDiscardHand(state, pd.player, cards, '宿屋の主人で');
        return state;
      }
      // 狩人＝アクション→財宝→勝利点 の順に1枚ずつ手札へ（強制）。
      case 'HUNTER_PICK': {
        const pd = state.pending;
        if (!pd || pd.type !== 'hunter_pick') return state;
        const owner = state.players[pd.player];
        const card = action.card;
        if (card == null || pd.cards.indexOf(card) < 0) return state;
        if (!hunterMatch(state, card, pd.stage)) return state;
        removeOne(pd.cards, card);
        owner.hand.push(card);
        log(state, `${owner.name} は狩人で「${C()[card].name}」を手札に加えた。`);
        hunterAdvance(state, pd);
        return state;
      }
      /* 要塞＝二択（+3コイン ／ 次の自分のターンの開始時に +3カード）。
         **後者を選んだときだけ持続になる**（条件つき持続＝貨物船と同型。無条件に予約を張ると片付けの仕分けが壊れる）。 */
      ...
      /* 堡塁＝コスト4以下を1枚獲得（強制）→ **獲得時効果を全部解決してから** 二択。
         獲得したカードが獲得先から動いていたら「手札に加える」は空振り（lose track＝公式FAQ逐語）。 */
      case 'HILL_FORT_GAIN': {
        const pd = state.pending;
        if (!pd || pd.type !== 'hill_fort_gain') return state;
        const card = action.card;
        if (card == null || !costUpTo(state, card, 4)) return state; // 強制
        const dest = 'discard';
        const real = mixedTopCard(state, card) || card; // 混合山なら実際に手に入るのは一番上の実カード
  ...
  const PLAYER_ACTIONS = new Set([
    ...
    // 同盟（Allies）A4：王国カード49種
    'ROTATE_PILE', 'TOWN_CHOOSE', 'BLACKSMITH_CHOOSE', 'MILLER_PICK', 'MARQUIS_DISCARD', 'SYCOPHANT_DISCARD',
    'SIBYL_PLACE', 'CAPITAL_CITY', 'INNKEEPER_CHOOSE', 'INNKEEPER_DISCARD', 'HUNTER_PICK',
