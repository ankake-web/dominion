...
console.log('=== 秘密の小部屋(アクション): 捨てた枚数だけ+1コイン ===');
{
  ...
  ok(s.pending && s.pending.type === 'secret_chamber' && s.pending.stage === 'discard', '捨てる選択待ち');
  const c0 = s.turn.coins;
  s = reduce(s, { type: 'SECRET_CHAMBER_RESOLVE', cards: ['estate', 'estate'] });
  ok(s.turn.coins === c0 + 2, '2枚捨てて+2コイン: ' + s.turn.coins);
  ok(count(s.players[0].discard, 'estate') === 2, '捨てた屋敷は捨て札へ');
  ok(s.pending === null, '解決完了');
}
{
  // 0枚捨ても可
  let s = setup(['secret_chamber', 'gold']);
  s = reduce(s, { type: 'PLAY_ACTION', card: 'secret_chamber' });
  const c0 = s.turn.coins;
  s = reduce(s, { type: 'SECRET_CHAMBER_RESOLVE', cards: [] });
...
console.log('=== 秘密の小部屋(リアクション): アタックに+2引き2枚戻す ===');
{
  ...
  let s = E.createInitialState(['A', 'B'], DOM.KINGDOM_INTRIGUE, { startActive: 0 });
  s.players[0].hand = ['torturer'];
  s.players[1].hand = ['secret_chamber', 'copper', 'copper', 'estate'];
  s.players[1].deck = ['gold', 'silver', 'duchy'];
  s = reduce(s, { type: 'PLAY_ACTION', card: 'torturer' });
  ok(s.pending && s.pending.type === 'torturer' && s.pending.player === 1, '拷問人: Bに選択待ち');
  const handBefore = s.players[1].hand.length; // 4
  s = reduce(s, { type: 'SECRET_CHAMBER_REVEAL' });
  ok(s.pending && s.pending.type === 'secret_chamber_putback' && s.pending.player === 1, '公開→戻す選択待ち');
  ok(s.players[1].hand.length === handBefore + 2, '+2カード引いた: ' + s.players[1].hand.length);
  s = reduce(s, { type: 'SECRET_CHAMBER_PUTBACK', cards: ['copper', 'copper'] });
  ok(s.players[1].deck[0] === 'copper' && s.players[1].deck[1] === 'copper', '2枚が山札の上に戻る');
  ok(s.pending && s.pending.type === 'torturer' && s.pending.reacted === true, '拷問人の解決へ復帰(reacted=true)');
  // reacted 後は再公開できない（無限ループ防止）
  const snap = JSON.stringify(s);
  const s2 = reduce(s, { type: 'SECRET_CHAMBER_REVEAL' });
  ok(JSON.stringify(s2) === snap, 'reacted後の再公開は無効(state不変)');
  // その後ふつうに拷問人を解決
  s = reduce(s, { type: 'TORTURER_RESOLVE', choice: 'discard', cards: [s.players[1].hand[0], s.players[1].hand[1]] });
