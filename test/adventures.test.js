...
/* ============================================================================
   E8：倒壊(raze) と 命令（Command）／および pending.self の後方互換
     - 命令（大君主/はみだし者/船長/王子）で動かさずに使った倒壊は「これ」を廃棄できない（公式2019エラッタ）。
     - pending.self は v44 で新設した。オンライン対戦の永続化スナップショット（v43以前）を復元すると
       self が欠落するので、engine.pendingSelf が旧来の意味（場に本体があれば廃棄できる）へ戻すこと。
       これが無いと「空手札の倒壊 pending が永久に閉じない（CPU livelock／人間が詰む）」。
   ============================================================================ */
console.log('=== E8: 倒壊 × 命令（はみだし者）と pending.self の後方互換 ===');
{ // はみだし者で倒壊を使う＝「これ」は廃棄できず、手札からのみ廃棄
  const K = ['band_of_misfits', 'raze', 'village', 'smithy', 'market', 'moat', 'cellar', 'militia', 'laboratory', 'festival'];
  let s = setup(K, ['band_of_misfits', 'estate'], ['copper', 'copper', 'copper']);
  s = reduce(s, { type: 'PLAY_ACTION', card: 'band_of_misfits' });
  s = reduce(s, { type: 'BAND_OF_MISFITS_PLAY', card: 'raze' });
  ok(s.pending && s.pending.type === 'raze' && s.pending.self === false, 'はみだし者×倒壊: pending.self=false');
  ok(E.pendingSelf(s, s.pending, 'raze') === false, 'pendingSelf も false');
  const before = JSON.stringify(s);
  s = reduce(s, { type: 'RAZE_TRASH', card: 'raze' });
  ok(JSON.stringify(s) === before, 'はみだし者×倒壊: 「これを廃棄」は engine が拒否');
  ok(CPU.decide(s, 0).card !== 'raze', 'はみだし者×倒壊: CPU も「これ」を提案しない');
  s = reduce(s, { type: 'RAZE_TRASH', card: 'estate' });
  ok(count(s.trash, 'estate') === 1 && count(s.trash, 'band_of_misfits') === 0, 'はみだし者×倒壊: 手札から廃棄・はみだし者は無事');
}
{ // 【回帰】v43 スナップショット互換：self を欠いた raze pending（手札0枚）が解決できること
  const K = ['raze', 'village', 'smithy', 'market', 'moat', 'cellar', 'militia', 'laboratory', 'festival', 'guide'];
  let s = setup(K, ['raze'], ['copper', 'copper', 'copper']);
  s = reduce(s, { type: 'PLAY_ACTION', card: 'raze' }); // 手札は空
  ok(s.pending && s.pending.type === 'raze' && s.players[0].hand.length === 0, '倒壊：手札0枚で廃棄 pending');
  delete s.pending.self;                                   // v43 の pending 形状を再現
  s = JSON.parse(JSON.stringify(s));                       // 永続化→復元の round-trip
  ok(E.pendingSelf(s, s.pending, 'raze') === true, '後方互換: self 欠落→場に本体があるので true');
  const act = CPU.decide(s, 0);
  ok(act && act.card === 'raze', '後方互換: CPU は「これを廃棄」を提案する');
  let g = 0; while (s.pending && g++ < 20) s = reduce(s, CPU.decide(s, 0));
  ok(!s.pending, '後方互換: pending が閉じる（livelock しない）');
  ok(count(s.trash, 'raze') === 1, '後方互換: 倒壊自身が廃棄された');
}
{ // 【回帰】v43 スナップショット互換：self を欠いた death_cart pending でも「自身を廃棄して+$5」が出る
  const K = ['death_cart', 'raze', 'village', 'smithy', 'market', 'moat', 'cellar', 'militia', 'laboratory', 'festival'];
  let s = setup(K, ['death_cart'], ['copper', 'copper', 'copper']);
  s = reduce(s, { type: 'PLAY_ACTION', card: 'death_cart' });
  delete s.pending.self;
  s = JSON.parse(JSON.stringify(s));
  ok(E.pendingSelf(s, s.pending, 'death_cart') === true, '後方互換: death_cart の self 欠落→true');
  s = reduce(s, { type: 'DEATH_CART_RESOLVE', mode: 'this' });
  ok(s.turn.coins === 5 && count(s.trash, 'death_cart') === 1, '後方互換: 自身を廃棄して +$5');
}
{ // 【回帰】v43 の fromCommand フラグが残っていれば「これ」は廃棄できない（はみだし者経由の中断復元）
  const K = ['death_cart', 'band_of_misfits', 'village', 'smithy', 'market', 'moat', 'cellar', 'militia', 'laboratory', 'festival'];
  let s = setup(K, ['death_cart'], ['copper', 'copper', 'copper']);
  s = reduce(s, { type: 'PLAY_ACTION', card: 'death_cart' });
  delete s.pending.self; s.pending.fromCommand = true;      // v43（はみだし者経由）の pending 形状
  s = JSON.parse(JSON.stringify(s));
  ok(E.pendingSelf(s, s.pending, 'death_cart') === false, '後方互換: fromCommand が立っていれば false');
  s = reduce(s, { type: 'DEATH_CART_RESOLVE', mode: 'this' });
