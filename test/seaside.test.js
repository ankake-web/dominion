...
console.log('=== 封鎖+堀: プレイ時に堀を公開した相手はこの封鎖の呪いから免疫 ===');
{
  let s = mkA(); s.players[0].hand = ['blockade']; s.players[1].hand = ['moat'];
  s = playFull(s, 'blockade'); // gain→脇置き→席1へ堀の反応窓（CPUが堀を公開）
  const gained = s.players[0].setAside[0];
  const bl = s.players[0].delayedEffects.find((e) => e.type === 'blockade');
  ok(bl && (bl.immune || []).includes(1), '封鎖+堀: 堀を公開した席1が免疫リストに入る');
  s = reduce(s, { type: 'END_ACTION_PHASE' }); s = reduce(s, { type: 'END_TURN' }); // →席1
  s = reduce(s, { type: 'END_ACTION_PHASE' }); // 席1 購入フェイズ
  const before = count(s.players[1].discard, 'curse');
  s.turn.coins = 8; s.turn.buys = 1;
  s = reduce(s, { type: 'BUY', card: gained });
  s = resolveAll(s);
...
console.log('=== 封鎖+灯台: 灯台で免疫の相手は反応不要で呪いを受けない ===');
{
  ...
  const gained = s.players[0].setAside[0];
  const bl = s.players[0].delayedEffects.find((e) => e.type === 'blockade');
  ok(bl && (bl.immune || []).includes(1), '封鎖+灯台: 灯台の席1が自動で免疫リストに入る');
  s = reduce(s, { type: 'END_ACTION_PHASE' }); s = reduce(s, { type: 'END_TURN' });
  s = reduce(s, { type: 'END_ACTION_PHASE' });
  const before = count(s.players[1].discard, 'curse');
  s.turn.coins = 8; s.turn.buys = 1;
...
console.log('=== 海賊(リアクション): 誰かが財宝を獲得すると手札から海賊を使える ===');
{
  let s = mkA(); // 席0の手番
  s.players[1].hand = ['pirate']; // 席1は手札に海賊
  s = reduce(s, { type: 'END_ACTION_PHASE' }); // 席0 購入フェイズ
  s.turn.coins = 3; s.turn.buys = 1;
  s = reduce(s, { type: 'BUY', card: 'silver' }); // 席0が財宝を獲得→席1に反応窓
  ok(s.pending && s.pending.type === 'pirate_react' && s.pending.player === 1, '海賊: 相手の財宝獲得で反応窓が席1に開く');
  s = reduce(s, { type: 'PIRATE_REACT', play: true });
  ok(s.players[1].inPlay.includes('pirate') && !s.players[1].hand.includes('pirate'), '海賊: リアクションで手札から場に出る');
  ok(!s.pending, '海賊: 反応窓を解決すると席0の購入フェイズへ戻る');
  // 席1の次の手番開始で海賊の持続が発火し、財宝を手札に獲得
  s = reduce(s, { type: 'END_TURN' }); // 席0→席1
  s = resolveAll(s);
  ok(s.turn.active === 1, '海賊: 席1の手番へ');
  ok(s.players[1].hand.some((c) => DOM.isType(c, 'treasure') && c !== 'copper'), '海賊: 反応で使った海賊が次手番に財宝を手札へ');
}

console.log('=== 海賊(リアクション): 使わない選択もできる（詰まらない）===');
{
  let s = mkA(); s.players[1].hand = ['pirate'];
  s = reduce(s, { type: 'END_ACTION_PHASE' });
  s.turn.coins = 3; s.turn.buys = 1;
  s = reduce(s, { type: 'BUY', card: 'silver' });
  ok(s.pending && s.pending.type === 'pirate_react', '海賊: 反応窓が開く');
  s = reduce(s, { type: 'PIRATE_REACT', play: false });
  ok(!s.pending && s.players[1].hand.includes('pirate'), '海賊: 使わないと海賊は手札に残り、窓は閉じる');
}

console.log('=== 海賊(リアクション): 複数人が持つと手番順に連鎖し、詰まらない ===');
{
  let s = mk(KA, ['A', 'B', 'C'], 0);
  s.players[0].deck = ['copper', 'copper', 'copper', 'silver', 'silver'];
  s.players[1].hand = ['pirate']; s.players[2].hand = ['pirate'];
  s = reduce(s, { type: 'END_ACTION_PHASE' });
  s.turn.coins = 3; s.turn.buys = 1;
  s = reduce(s, { type: 'BUY', card: 'silver' }); // 席0の獲得→席1・席2に順に窓
  ok(s.pending && s.pending.type === 'pirate_react' && s.pending.player === 1, '海賊: まず席1へ窓');
  s = reduce(s, { type: 'PIRATE_REACT', play: true });
  ok(s.pending && s.pending.type === 'pirate_react' && s.pending.player === 2, '海賊: 次に席2へ窓が連鎖');
  s = reduce(s, { type: 'PIRATE_REACT', play: false });
  ok(!s.pending, '海賊: 全員解決で窓が閉じ、購入フェイズへ戻る');
  ok(s.players[1].inPlay.includes('pirate') && s.players[2].hand.includes('pirate'), '海賊: 席1は使用・席2は温存が正しく反映');
}
