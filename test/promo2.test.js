...
console.log('=== 新プロモ: CARD_SET 昇格・現行エラッタ種別 ===');
{
  ...
  ok(DOM.POOLS.promo.length === 12, 'プロモプールは12種（境界地 marchland が合流／隠し財産 hoard は繁栄のカードなので外した）');
  ok(!DOM.POOLS.promo.includes('hoard'), '隠し財産はプロモのプールに入っていない（本来は繁栄のカード）');
  ok(DOM.POOLS.prosperity.includes('hoard'), '隠し財産は繁栄のプールにある');
...
console.log('=== 船長: 鉱山の村をサプライからプレイ→自己廃棄は失敗し+2コインも出ない ===');
{
  ...
  // E8：命令が動かさずに使った鉱山の村は「これ」を廃棄できない＝死に選択肢を出さない（選択自体が出ない）。
  ok(!s.pending, '「これを廃棄する？」の選択は出ない（廃棄できないので）');
  ...
  ok(count(s.trash, 'captain') === 0 && count(s.players[0].inPlay, 'captain') === 1, '船長も廃棄されず場に残る');
}
console.log('=== 船長: 場に本物の鉱山の村があってもサプライのそれをプレイして誤爆しない（E8回帰） ===');
{
  const K2 = ['mining_village', 'sauna', 'village', 'moat', 'militia', 'smithy', 'market', 'stash', 'church', 'captain'];
  let s = setup(K2, ['captain', 'mining_village', 'copper', 'copper', 'estate'], ['copper', 'copper']);
  s = playAct(s, 'mining_village');
  s = reduce(s, { type: 'MINING_VILLAGE_RESOLVE', trash: false }); // 本物は廃棄せず場に残す
  ok(count(s.players[0].inPlay, 'mining_village') === 1, '本物の鉱山の村が場にある');
  s = playAct(s, 'captain');
  s = reduce(s, { type: 'CAPTAIN_PLAY', card: 'mining_village' });
  ok(!s.pending && count(s.trash, 'mining_village') === 0, '場の本物を誤って廃棄しない');
  ok(count(s.players[0].inPlay, 'mining_village') === 1, '本物は場に残ったまま');
}
...
/* ============ E8：王子（脇に置いたまま使用）＝脇のカードの自己移動は失敗 ============
   公式（2019エラッタ）：命令カードは対象を「動かさずに」プレイするだけ。対象は脇に残り、
   「これを廃棄／マットに置く／山へ戻す」は失敗する。王子自身も身代わりに動かない。
   ただし「手札から」のように場所が明示された移動は普通に起きる。 */
console.log('=== E8: 王子 × 自己移動札（脇に残り、毎ターン再生され続ける） ===');
{ // 王子 × 島：島は脇に残り、毎ターン手札1枚が島マットへ
  const K = ['prince', 'island', 'village', 'moat', 'militia', 'smithy', 'market', 'stash', 'church', 'sauna'];
  let s = setup(K, ['prince', 'island', 'copper', 'copper', 'estate'], ['copper', 'copper', 'copper', 'copper', 'copper']);
  const is0 = s.supply.island;
  s = playAct(s, 'prince');
  s = reduce(s, { type: 'PRINCE_SETASIDE', card: 'island' });
  ok(count(s.players[0].princes, 'island') === 1, '王子×島：島を脇へ');
  s = endTurnAndPass(s);
  ok(s.pending && s.pending.type === 'prince_play', '次ターン開始時に王子が脇の島を使う');
  s = reduce(s, { type: 'PRINCE_PLAY' });
  ok(s.pending && s.pending.type === 'island', '「手札から1枚」を島マットへ置く選択が出る');
  const hc = s.players[0].hand[0];
  s = reduce(s, { type: 'ISLAND_PICK', card: hc });
  ok(count(s.players[0].islandMat, hc) === 1, '王子×島：手札1枚は島マットへ（場所が明示された移動）');
  ok(count(s.players[0].islandMat, 'island') === 0 && count(s.players[0].islandMat, 'prince') === 0, '王子×島：島も王子も島マットに乗らない');
  ok(count(s.players[0].princes, 'island') === 1, '王子×島：島は脇に残り続ける');
  ok(count(s.players[0].durationCards, 'prince') === 1, '王子×島：王子も場に残り続ける');
  ok(s.supply.island === is0, 'サプライの島は減らない');
}
{ // 王子 × 鼠取り（Reserve）：酒場マットに乗らない（+1カード+1アクションは入る）
  const K = ['prince', 'ratcatcher', 'village', 'moat', 'militia', 'smithy', 'market', 'stash', 'church', 'sauna'];
  let s = setup(K, ['prince', 'ratcatcher', 'copper', 'copper', 'estate'], ['copper', 'copper', 'copper', 'copper', 'copper']);
  s = playAct(s, 'prince');
  s = reduce(s, { type: 'PRINCE_SETASIDE', card: 'ratcatcher' });
  s = endTurnAndPass(s);
  const h0 = s.players[0].hand.length, a0 = s.turn.actions;
  s = reduce(s, { type: 'PRINCE_PLAY' });
  ok((s.players[0].tavern || []).length === 0, '王子×鼠取り：酒場マットに乗らない（自己移動は失敗）');
  ok(s.players[0].hand.length === h0 + 1 && s.turn.actions === a0 + 1, '王子×鼠取り：+1カード+1アクションは入る');
  ok(count(s.players[0].princes, 'ratcatcher') === 1, '王子×鼠取り：鼠取りは脇に残り続ける');
}
{ // 王子 × 鉱山の村：自身を廃棄できない＝+$2 は出ない（選択も出ない）。王子も廃棄されない
  const K = ['prince', 'mining_village', 'village', 'moat', 'militia', 'smithy', 'market', 'stash', 'church', 'sauna'];
  let s = setup(K, ['prince', 'mining_village', 'copper', 'copper', 'estate'], ['copper', 'copper', 'copper', 'copper', 'copper']);
  s = playAct(s, 'prince');
  s = reduce(s, { type: 'PRINCE_SETASIDE', card: 'mining_village' });
  s = endTurnAndPass(s);
  const c0 = s.turn.coins;
  s = reduce(s, { type: 'PRINCE_PLAY' });
  ok(!s.pending, '王子×鉱山の村：廃棄の選択は出ない（廃棄できないので）');
  ok(s.turn.coins === c0, '王子×鉱山の村：+$2 は出ない');
  ok(count(s.trash, 'mining_village') === 0 && count(s.trash, 'prince') === 0, '王子×鉱山の村：どちらも廃棄されない');
  ok(count(s.players[0].princes, 'mining_village') === 1 && count(s.players[0].durationCards, 'prince') === 1, '脇のカードも王子も残る');
}
{ // 王子 × 狂人：狂人の山へ戻らない＝ドローなし（+2アクションのみ）
  const K = ['prince', 'hermit', 'village', 'moat', 'militia', 'smithy', 'market', 'stash', 'church', 'sauna'];
  let s = setup(K, ['prince', 'madman', 'copper', 'copper', 'estate'], ['copper', 'copper', 'copper', 'copper', 'copper']);
  const md0 = s.supply.madman;
  s = playAct(s, 'prince');
  s = reduce(s, { type: 'PRINCE_SETASIDE', card: 'madman' });
  s = endTurnAndPass(s);
  const h0 = s.players[0].hand.length, a0 = s.turn.actions;
  s = reduce(s, { type: 'PRINCE_PLAY' });
  ok(s.supply.madman === md0, '王子×狂人：狂人は山へ戻らない');
  ok(s.players[0].hand.length === h0, '王子×狂人：ドローなし（If you did が偽）');
  ok(s.turn.actions === a0 + 2, '王子×狂人：+2アクションは入る');
  ok(count(s.players[0].princes, 'madman') === 1, '王子×狂人：狂人は脇に残る');
}
