...
- 発売＝**2022年12月19日**。**印刷は初版のみ（第2刷は無い）**＝2022年12月のテキストが現行。
  機能エラッタは **Journey 1枚だけ**（2023年9月 Extra turn errata・**未印刷**）＝**採用すると決定した**（下記 D1）。
...
## ✅ この研究で下した決定（2026-08-15）

### D1. **旅行(Journey)は 2023年9月「Extra turn errata」側を採用する**（＝新しい版）
現行テキスト＝`You don't discard cards from play in Clean-up this turn. Take an extra turn after this one (but not a 3rd turn in a row).`
→ **`ONCE_PER_TURN_EVENTS` には登録しない**（`Once per turn:` の前置句はエラッタで消えた。
このリストに入るのは **`launch` だけ**）。「3ターン連続にはできない」は
**島民・航海と同じ既存の共通機構**（§0-29 A5）にそのまま乗せる。

**根拠（この順で重い）**
1. **本プロジェクトは同じ2023エラッタの `mission`（使節団）を既に採用済み**。
   `js/engine.js` の `ONCE_PER_TURN_EVENTS` は `['alms','borrow','save','pilgrimage','desperation']` で
   **`mission` が入っていない**（§0-26＝「現行エラッタで前置句が消えた」として外した）。
   **旅行は使節団の完全な同型**（どちらもイベント／`Once per turn:` が消えた／追加ターン）。
   ここだけ旧版にすると**同じエラッタの中で挙動が割れる**。
2. **決定 D2（日本語文面は Dominion Online 訳で統一）と整合しない**。
   日本語wiki（＝DO訳）の `旅行` は**エラッタ側の文面**
   （`このターンのクリーンアップフェイズに場のカードを捨て札に置かない／その後追加ターンを得る
   (ただし、連続3ターンとなる場合は得られない)`）。
   2022印刷版を採ると**「表示は新版・挙動は旧版」**になり、カードがプレイヤーに嘘をつく。
3. **ユーザーは日本語版カードの現物を持っていない**（メモリ `no-physical-dominion-cards`）＝
   「印刷物と一致させる」ことに実利が無く、**実際に触れる公式デジタル（＝エラッタ側）に合わせるほうが良い**。
4. **「3ターン連続不可」は engine に既にある**（島民・航海）。旧版の「直前のターンがあなたのものでない場合」は
   **engine に無い別機構**なので、旅行1枚のために新設することになる。

⚠ **反対材料も残しておく**＝英語wiki の `Versions` 表で**エラッタ行の Print 欄は `Not printed yet`**（未印刷）で、
wiki はこの版のFAQ節を **`Unofficial FAQ (Post-2023 Errata)`** と呼んでいる。
**§0-29 A4 の royal_galley では「未印刷のアナウンスは採らない」と決めた**＝形式だけ見ると逆の判断になる。
ただし royal_galley は *announce* のみで**公式エラッタ文書にも wiki の `Versions` 表にも行が無かった**のに対し、
旅行は**正式なエラッタ行がある**（`Changes = Never allow more than two turns in a row. / Announced = September 2023`）。
**反転は1行**（`ONCE_PER_TURN_EVENTS` に `journey` を足し、カタログ文と条件節を旧版に戻す）。

### D2. **日本語のカード文面は Dominion Online 訳（日本語wiki 掲載）で85種を統一する**
- **印刷版の逐語を85種ぶん揃える手段が無い**（英語wiki に Japanese 行があるのは Crew / Invasion / Siren / Taskmaster の
  4枚だけ・215×344px の低解像度・**掲載テキストに誤字**〈`廃楽`／`緩得`〉）。
  4枚だけ印刷版に寄せると**不統一＋誤字混入**になるので採らない。
- **日本語の「名前」はどちらの資料でも一致**しているので、名前の確度は高い（＝ここは安心して確定してよい）。
- ⚠ **将来、実物カードが手に入ったら差し替える**（差分は表記のみ・機能差はゼロ）。

### D3. **港の村(Harbor Village)は、習性(Way)由来の +$ ではボーナスを「得ない」**
- 英語wiki の `Official FAQ` は `Using a Way ... does get you the bonus.` と書いているが、
  **これは2022年の公式FAQ文書の再録**であり、**2025年2月の移動動物園エラッタより前の文**。
- **現行の `Way` ページの逐語**＝`When you play an Action card, **instead of doing what is printed on the Action card**,
  you may do what is printed on a Way` ＝**+$ を出しているのは習性であってアクションカードではない**。
- **同じ FAQ の原則がそれを支持する**＝`This only cares if **the Action itself** gave you +[$], not if you otherwise got
  +[$] due to playing it (such as due to Training ... or The Forest's Gift ...).`
  ＝山トークンや祝福で得た +$ を除外するのと**同じ理屈**で、習性由来も除外されるのが自然。
- **本プロジェクトは2025年2月エラッタを既に採用済み**（§0-25/§0-26 で戦車競走・使節団・儀式を修正）。
  日本語wiki も同エラッタ＋2025年3月の Donald X. の Discord 回答を出典に「得られない」と明記している。
- ⚠ **mix-all でしか到達しない**（移動動物園の習性＋略奪の港の村）。**反転は1行**。

### D4. **旗艦(Flagship)は持続を再演したら場に残す（例外を設けない）**
- **一般則＝残る**（RGG ルールブック逐語＋現行 `Plunder (expansion)` の追加ルール＋`Duration` ページ）。
  逐語＝`if a Duration card is played extra times by a card such as [Throne Room, Scepter, Mastermind, Specialist,
  Flagship, or Daimyo], that card also stays in play until the Duration card is discarded`。
  **同盟の専門家(Specialist)と同じ実装**にできる。
- ⚠ **これは §0-25/§0-28 の「玉座×持続では玉座が残らない」という既存の許容簡略化を、旗艦では採らない**という意味。
- **「はみだし者経由だけは残らない」という脚注付き裁定は、2025-09-11 の版で英語wikiから削除済み**＝
  **現行資料に例外が無いので例外を設けない**（本プロジェクトの「現行に寄せる」方針どおり）。
  ⚠ 削除が「誤りだったから」か「2025年9月の一括編集の巻き添え」かは一次資料から断定できない。
  理屈の上では例外に分がある（命令が使わせる持続は**サプライから動かない＝一度も場に入らない**ので追跡が成立しない）。
  **必要なら分岐1つで例外を足せる。**

### D5. **【ユーザー決定】`spoils`（暗黒時代）の日本語名を、公式訳の「略奪品」へ改名する。**
...
- **版＝2023 Extra turn errata を採用**（決定 D1）。
...
- **持続を再演した場合、旗艦は「その持続が場から捨てられるまで」場に残る**（RGG ルールブック逐語＋現行 wiki）。
  ...
  2023〜2025年9月の英語wikiに脚注付きで存在したが**2025-09-11 の版で削除**＝**決定 D4＝例外を設けない**。
...
## ⚠️ 版の選択・裁定が割れた論点（**すべて決着済み。上の「この研究で下した決定」を見ること**）

| # | 項目 | 決定 | 反転コスト |
|---|---|---|---|
| 1 | `journey`（旅行）の版 | ✅ **D1＝2023 Extra turn errata を採用**（`ONCE_PER_TURN_EVENTS` に登録するのは `launch` だけ・3ターン連続不可） | **1行＋カタログ文** |
| 2 | 日本語カード文面の版 | ✅ **D2＝Dominion Online 訳で85種統一**（名前は両資料で一致＝確度は高い） | カタログ差し替え |
| 3 | `spoils` の日本語名 | ✅ **D5＝「略奪品」へ改名（実施済み・`ea0c091`）** | 実施済み |
| 4 | 港の村×習性(Way)由来の +$ | ✅ **D3＝得ない**（現行の `Way` の逐語＋2025年2月エラッタ側。mix-all 限定） | **1行** |
| 5 | 旗艦×はみだし者で旗艦が場に残るか | ✅ **D4＝一般則どおり常に残す（例外を設けない）** | 分岐1つ |

**⚠ 各章の本文には「要ユーザー確認」と書かれたまま残っている箇所がある**（章は調査時点のスナップショット）。
**上の決定が優先**。章の記述は「なぜ割れていたか」の根拠として読むこと。
**残る未確認はすべて実装に効かないもの**（印刷カードの改行位置・イラストレーター・特性14種の印刷版和文 など）。
