...
- **2026-08-13 時点＝15拡張＋プロモ＝全676枚（縦型505＋横型171）が実プレイ可能**
  （基本・陰謀・海辺・錬金術・繁栄・収穫祭・ギルド・異郷・暗黒時代・冒険・帝国・ルネサンス・移動動物園・夜想曲・**同盟**
   ＋プロモ／新プロモ。横型＝イベント・ランドマーク・プロジェクト・アーティファクト・習性・祝福/呪詛/状態・同盟カード）。
  拡張を自由に混ぜる **mix-all** もある。**残りは 略奪／日の出づる国 の2拡張**
  （**略奪は段階0＝研究＋段階1＝カタログ85種＋webp まで完了**＝正本 `docs/research/plunder_rules.md`。
  `CARD_SETS` 未参照なのでまだ実プレイには出ない）。
