...
const MAX_ROOMS = 2000;       // 同時部屋数の上限。多数接続でメモリ/コード空間(1万)を枯渇させる軽微DoSを防ぐ。
...
/* mix モード（拡張を自由に混ぜる）＝セットIDが `mix:<王国プール>[:<横型枚数>[:<横型プール>]]` の文字列。
   CARD_SETS に無い動的IDなので、**形式と各プール名の実在を検証してから受理する**（不正な文字列で
   kingdomForSet がフォールバックし、ホストの意図しない王国で開始するのを防ぐ）。 */
function isValidKingdomSet(id) {
  if (typeof id !== 'string' || id.length > 300) return false;
  if (KINGDOM_SETS.includes(id)) return true;
  if (!DOM.isMixSet || !DOM.isMixSet(id)) return false;
  if (!/^mix:[a-z0-9_]+(,[a-z0-9_]+)*(:[0-2](:[a-z0-9_-]+(,[a-z0-9_-]+)*)?)?$/.test(id)) return false;
  const m = DOM.parseMixSet(id);
  // parseMixSet は不正なプール名を捨てるので、捨てられた＝知らないプール名が混じっていた、と判定する。
  const raw = id.slice(4).split(':');
  const rawPools = (raw[0] || '').split(',').filter(Boolean);
  const rawLs = (raw[2] || '').split(',').filter(Boolean);
  if (rawPools.length !== m.pools.length || !m.pools.length) return false;
  if (rawLs.length !== m.lsPools.length) return false;
  return true;
}
...
function startGame(room) {
  ...
  // 王国カード（基本/陰謀/ランダム/mix）。ランダム系はサーバ権威で1度だけ確定し全員で共有する。
  ...
  const ls = DOM.landscapesForSet ? DOM.landscapesForSet(room.kingdomSet || 'basic') : { landmarks: [], events: [], projects: [] };
  const landmarks = ls.landmarks, events = ls.events, projects = ls.projects;
...
function handleConnection(ws) {
  ...
  ws.on('message', (data) => {
    ...
    try {
      ...
      case 'create': {
        if (room) return;
        if (rooms.size >= MAX_ROOMS) { send(ws, { t: 'error', message: '混雑しています。しばらくしてから再試行してください' }); return; }
        let code;
      ...
      case 'setConfig': {
        ...
        if (isValidKingdomSet(msg.kingdomSet)) room.kingdomSet = msg.kingdomSet;
