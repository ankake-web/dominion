...
### 遊べるモード
- **対戦をはじめる**：2〜4人。各席を「人間 / CPU（弱・普通・強）」に自由割り当て（混在OK）。CPUは自動で進みます。
- **1台で2人プレイ（クイック）**：1台を回して2人で対戦（相手の番は手札を隠す画面をはさむ）。
- **オンラインで対戦（2台）**：部屋コードで2台のスマホで対戦（Firebase設定が必要：フェーズ2）。
- **遊び方 / カード一覧**：ルール説明と、全カードの一覧（タップで拡大・コスト/効果つき）。
...
├── css/style.css
├── js/                  ← ゲームのプログラム（engine/cpu/store/ui）
├── assets/              ← カード画像（<カード名>.png 原本 + 軽量 .jpg）
...
- すぐにCPUと遊ぶなら「**対戦をはじめる**」→ そのまま「この設定で開始」（あなた vs CPU普通）。
- 人と2人で遊ぶなら「**1台で2人プレイ（クイック）**」。
...
## 開発者向け：テスト
```bash
npm test                     # エンジン/CPU/UI/オンライン をまとめて実行（210項目）
node test/engine.test.js     # ルールエンジン（多人数・各カード）
node test/cpu.test.js        # CPU対局（終了性・強さの序列）
node test/ui.test.js         # 画面操作の統合テスト（jsdom）
node test/online.test.js     # オンライン同期（モックFirebase）

# 見た目・レスポンシブの実機確認（Chromiumでスクショ＋はみ出し検査）
npm run serve &              # 先にローカルサーバーを起動
npm run verify:visual        # 360/768/1280px で検証
```
