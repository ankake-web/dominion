...
- ✅ **A5＝CARD_SET 昇格**（このセッション・**未push**）＝`allies`（固定10種）／`random-allies`／
  mix-all（14→15拡張）／`DOM.STAGE1_POOLS` を空に（**闇市場に同盟の非分割25種が入る**）／
  CPU の `GAIN_ORDER` を実強度順へ／**多エージェント敵対レビュー確定10件を全修正**。
- ⚠ **未push のコミットが1つある**（`feat(allies): A5＝CARD_SET 昇格…`）。**うち2件は出荷済みの既存バグ修正**
  （混合山から獲得したログが「獲得後の一番上」を名乗る＝**城は帝国で毎回ズレる**／320px で「ランダム」の
  抽選元セグ18個が幅22pxに潰れて縦書きになる CSS バグ）。**push はユーザー確認の上で。**
