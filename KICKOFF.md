...
## 最初にやること
1. `npm test` を実行し **全40スイート緑（exit 0・整合性 5303・不変条件 12）** を確認する。
...
## 次に取り組むタスク＝**略奪の段階2（engine/CPU/UI）＝P1 から**

バッチ順は正本の §「段階2 の推奨バッチ順」：
1. **P1＝Loot の山の基盤**（`state.loot` 30枚・獲得/公開/戻す・`maskStateFor` で全部伏せる・
   非サプライの除外4系統・`test/invariants.test.js` の tally に `(s.loot||[]).forEach(add)`）＋**Loot 15種**。
...
- **client 資産（js/css/webp 等）を変えたら `sw.js` の VERSION を上げる**（現在 **v72**）。
