...
## 最初にやること
1. `npm test` を実行し **全42スイート緑（exit 0・整合性 5303・不変条件 12・略奪136＋略奪UI 37）** を確認する。
...
## いまどこ
**15拡張＋プロモ＝全676枚が実プレイ可能。略奪(Plunder)は段階0（研究）＋段階1（カタログ85種＋webp）＋
段階2 P1（戦利品の山＋15種）まで完了・push済**
（`sw.js` **v74**・本番反映を機械照合済み＝Pages の主要4ファイルが sha1 一致／Render 稼働中／**未pushなし**）。
...
## 次に取り組むタスク＝**略奪の段階2 P2（"next time" 型持続の共通機構）から**

バッチ順は正本の §「段階2 の推奨バッチ順」：
1. ~~**P1＝Loot の山の基盤＋戦利品15種**~~ ✅**完了（`186e368` / `393baba` / `0c8925c` / `4f5caf2`）**＝
   `state.loot` 30枚／`gainLoot()`／非サプライ4系統除外／完全マスク／保存則／サーバの同意なし Undo／
   盤面の残枚数／**戦利品15種すべての効果**。`test/plunder.test.js` **136件**＋`plunder-ui.test.js` **37件**。
...
- **client 資産（js/css/webp 等）を変えたら `sw.js` の VERSION を上げる**（現在 **v74**）。
