...
## 現状：帝国（Empires）段階2＝Batch E1（負債）＋E2（単独9枚）＋E3（集合）＋E4（分割山5組）＋E5（城8）まで完了
- **Batch E5 完了**＝城8種を1つの混合山 `state.castles`（knights 流用・昇順に積む・一番上だけ購入/獲得・2人8枚/3-4人12枚）＋可変VP（humble=城数／kings=城数×2・自身含む全城）＋各on-gain/on-trash を実装。詳細＝PROGRESS §0-14。`sw.js` v40→**v41**。`test/empires.test.js` を全140件に拡張。**カタログ/webp変更なし**（研究WFで全8枚のカタログ＝公式一致・humble_castle に coin:1 のみ追加）。integrity 3134→**3144**（'castles' プレースホルダ card ぶん）。
...
- **帝国はまだ CARD_SET 未昇格＝本番挙動は不変**（帝国カードはサプライに出ない）。**E1〜E4は push 済**（sw.js v40）。その上に **Batch E5＋この handoff が未push**。
...
- **未pushコミットあり**（Batch E5＋この handoff。E1〜E4は push 済）。push は必ずユーザー確認。帝国は CARD_SET 未昇格＝**本番挙動は不変**（帝国カードはサプライに出ない）。
