...
③段階2 **A1**（好意トークンと Ally 選定）④段階2 **A2**（分割山6組＝混合山モデル＋循環 Rotate）
⑤**A2b**（「カードの種別」を山の一番上で判定する横断修正）まで完了・**push 済**（`sw.js` v65）。
⑥**A3＝同盟(Ally)カード23種**（盤面表示＋好意の使用窓＋全効果）を実装し、敵対レビュー確定7件も修正
＝**すべて push 済**（`sw.js` **v66**・本番反映を実機確認）。うち **[high] 1件は A3 以前からあった UI の穴**
（選択のターン跨ぎ持ち越し）＝**地下貯蔵庫/民兵/拷問人など既存カードにも効く実バグ修正**。
**作業ツリーはクリーン／未pushなし。**
`npm test` **全40スイート緑**（整合性4763・不変条件12・**同盟310件**＋**同盟UI 99件**）／`verify:e2e` 9/9（webp 505/0）／
`verify:visual` はみ出し0。**同盟はまだ CARD_SETS 未参照＝実プレイには出ない**（`DOM.STAGE1_POOLS` で闇市場からも除外）。
**次にやること＝A4＝王国カード49種**（詳細は下の「段階2 A3」節と「注意（A3 固有）」＋`docs/research/allies_rules.md`）。
**同盟自体はまだ CARD_SETS 未参照＝実プレイには出ない**（闇市場にも出ないよう `DOM.STAGE1_POOLS` で明示的に塞いである）。
ただし **A2/A2b の敵対レビューで見つかった「同盟と無関係な既存バグ 計12件」を直したので、本番の挙動はその分だけ変わった**
（闇市場に混合山のプレースホルダが漏れて幻のカードが買えた／オンラインの1手もどすで伏せ札の騎士山を覗けた／
汚された神殿のVPが帝国の分割山下段で孤児化していた／冒険の山トークンが財宝と「アクション権を使わない使用」で乗らなかった／
取り替え子が分割山の札を交換できなかった／獲得モーダルと拡大シートがプレースホルダを描いていた／
種別ラベルが「アクション・アタック・」と欠けていた／塩まき×騎士の3面破れ／宣言モーダルのプレースホルダ）。
**すべて公式へ寄せる方向**。
`npm test` **全39スイート緑**（exit 0・整合性 **4763**・不変条件 **12**・**同盟169件**）／
`verify:e2e` 9/9（webp **505**/0）／`verify:visual` はみ出し0。
実装済みは 縦型505＋横型171＝**676枚**（うち同盟78枚は枠＋文字のみ＝**絵は未回収**）。
**【次にやること】A3＝Ally 23種**（`state.ally` は A1 で決定済み＝残るは盤面表示＋好意の使用窓＋23種の効果）
**→ A4＝王国カード49種 → A5＝CARD_SET 昇格（`DOM.STAGE1_POOLS` から `'allies'` を外す）＋絵72枚の回収。**
