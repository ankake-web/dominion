...
### 【次にやること】
1. **絵（webp）71枚の回収**（現在は枠＋文字の暗い板＝縦型31・横型40）。ChatGPT で生成 → `asset/art/<id>.png` →
   縦型 `CARDS_ONLY=<ids> node tools/build-cards.js`／横型 `CARDS_ONLY=<ids> node tools/build-landscape.js`
   （**このPCのみ**）。手順は記憶 `chatgpt-card-art-workflow`。
2. **ユーザー確認の上で push**（`sw.js` v57）。
3. その後＝**発売順の未着手拡張**（夜想曲／同盟／略奪／日の出づる国）。
...
**【現在地】§0-26＝新拡張「移動動物園（Menagerie）」が完成した（全71枚＝王国30＋馬＋習性20＋イベント20）。
`menagerie` / `menagerie-ways` / `menagerie-events` / `random-menagerie` が実プレイ可能。`sw.js` v57・**未push**。
...
【次にやること】①**絵（webp）71枚の回収**（縦型31＋横型40。現在は枠＋文字の暗い板）→ ②ユーザー確認の上で push
→ ③発売順の未着手拡張（夜想曲／同盟／略奪／日の出づる国）。詳細と落とし穴は **§0-26 と §0-25** に全部書いてある。**
...
1. **【最優先】移動動物園の絵（webp）71枚を回収する**（engine は §0-26 で完成済み。残るのは絵だけ）。
   現在は**枠＋文字の暗い板**（縦型31＝王国30＋馬／横型40＝習性20＋イベント20）。
   ChatGPT で生成 → `asset/art/<id>.png` に回収 → 縦型 `CARDS_ONLY=<ids> node tools/build-cards.js`／
   横型 `CARDS_ONLY=<ids> node tools/build-landscape.js`（**このPCのみ**）。手順は記憶 `chatgpt-card-art-workflow`。
   絵の有無は webp のファイルサイズで見分けられる（枠＋文字だけ＝36〜60KB／絵入り＝83〜125KB）。
2. **push（ユーザー確認の上で）**＝`sw.js` v57。push すれば本番 Pages/Render に移動動物園の全71枚が出る
   （サーバは `DOM.CARD_SETS` から `menagerie-events` を自動受理する＝サーバ側コード変更は不要）。
3. **その次の拡張候補**（着手前に `docs/adding-cards.md` 必読）:
   - ✅**絵の回収が済んでいる拡張**＝ルネサンス50枚（2026-07-13）／冒険イベント20種（2026-07-14）／
     帝国ランドマーク21種（2026-07-11）／帝国イベント13種（2026-08-07）＝**移動動物園の71枚以外は全部 絵入り**。
