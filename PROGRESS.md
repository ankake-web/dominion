...
**【現在地】§0-29＝新拡張「同盟（Allies）」に着手。公式ルール研究（多エージェント22体）と段階1（カタログ72種＋webp78枚）が完了。
`sw.js` v63・未push。CARD_SETS 未参照＝実プレイには出ない（闇市場にも出ないよう `DOM.STAGE1_POOLS` で明示的に塞いだ）。
`npm test` **全38スイート緑**（exit 0・整合性 4184→**4763**・不変条件10）／`verify:e2e` 9/9（webp **505**/0）／
`verify:visual` はみ出し0。実装済みは 縦型505＋横型171＝**676枚**（うち同盟78枚は枠＋文字のみ＝絵は未回収）。
**【次にやること】段階2。まず ★占星術師団／メイソン団と `reshuffleDeck`（同期・非対話）の衝突をどう解くか決める★**
（§0-29 の「次にやること」1番）。落とし穴は **§0-29 の注意節**と **`docs/research/allies_rules.md` 冒頭の「実装前に必読」9項目**。**
