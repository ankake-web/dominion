...
## 0-24. **UX 6件＋実バグ2件（1手もどす／終局後デッキ公開／カード検索／植民地終了／ティアラの出す順）**（2026-08-06・**未コミット・未push**・`sw.js` v53）
...
### 検証
- **npm test 全34スイート緑（exit 0）**：`prosperity` 69→**92**（植民地終了・deckCards・ティアラ/冠/偽造通貨の出し順＋CPUソーク6戦）／
  `ui` 82→**112**（検索6件・終局デッキ確認6件・LocalStore履歴7件・盤面の1手もどす8件）／`server` 49→**62**（オンラインUndoの承認/拒否/
  非手番拒否/巻き戻し配信/履歴を永続化しない）。
- `verify:e2e` 9/9（webp 371/0・例外0）／`verify:visual` 320〜768px 全幅はみ出し0。
...
**【現在地】§0-24＝UX6件＋実バグ2件（1手もどす／終局後デッキ公開／カード検索／植民地の終了条件／ティアラの出す順）を実装。
**未コミット・未push**（`sw.js` v53）。全34スイート緑（exit 0）／`verify:e2e` 9/9（webp 371/0）／`verify:visual` はみ出し0。
