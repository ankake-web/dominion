...
最終更新: 2026-07-01 / branch `main`。**海辺の簡略化2点（封鎖の堀免疫窓・海賊の財宝獲得リアクション）を本格実装＋混成王国のCPU購入バランス調整＝コミット&push済（`8235b32`）。Pages/Render 自動デプロイ実行中**。
公開: GitHub Pages https://ankake-web.github.io/dominion/ （クライアント）＋ Render（オンライン対戦サーバ）。
**新セッションは まず `npm test` を実行し 1856件オールグリーンを確認**してから着手すること（17スイート）。作業ツリーはクリーン・origin/main と同期。
