...
最終更新: 2026-07-10 / branch `main`（最新は `git log` で確認）。**帝国（Empires）は E7＝CARD_SET昇格まで完了＝縦型36枚すべて実プレイ可能・push済（`15b605e`）**（§0-16）。**続く E8＝命令(Command)の忠実化（＝「命令がプレイした札は動かない」）も完了・`52bba46` を push 済＝本番反映を実機確認済み**（§0-17。`sw.js` v43→**v44**・`asset/cards/overlord.webp` 再生成）。以後の拡張も 完成→CARD_SET昇格→全テスト緑→**都度ユーザー確認の上で** push（勝手に push しない）。
...
1. **【最優先】横型ランドスケープ or E9**（下の 2. 参照）。※**E8 は完了・push済**。E8 は「命令カードが身代わりに動く」ではなく「命令がプレイした札は動かない（現行2019エラッタ）」で実装した。旧計画の記述は §0-17 で訂正済み。
