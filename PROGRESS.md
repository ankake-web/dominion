...
**【現在地】§0-30＝新拡張「略奪（Plunder）」＝**段階0（研究）→段階1（カタログ85種＋webp）→段階2 P1〜P7 まで完了＝
**略奪85種が実プレイ可能になった**（`sw.js` v75・**未push**）。
`CARD_SETS` に **`plunder` / `plunder-events` / `plunder-traits` / `random-plunder`**、mix-all も **15→16拡張**。
＝**実装済み16拡張＋プロモ＝761枚（縦型560＋横型201）が実プレイ可能**（**絵は略奪85枚だけ未回収**＝枠＋文字）。
...
`npm test` **全42スイート緑**（exit 0・整合性**5307**・不変条件12・略奪**455**＋略奪UI **148**）／
`verify:e2e` 9/9（webp 560/0）／略奪の webp 85/85 存在を機械検算。
⚠ **2026-08-15 は2つのセッションが並行して同じリポジトリを触り、矛盾するコミットが出た**（§0-30 の「経緯」節）。
**【次にやること】①多エージェント敵対レビュー（4観点）の確定分を修正 → ②絵85枚の回収 → ③ユーザー確認の上で push。**
その後は**日の出づる国（Rising Sun）＝最後の1拡張**。**
