...
最終更新: 2026-08-11 / branch `main`（最新は `git log` で確認）。

**【現在地】§0-28＝新拡張「夜想曲（Nocturne）」の段階2が完成（未push・`sw.js` v61）。
全77種（王国33＋家宝7＋非サプライ5＋ゾンビ3＋祝福12＋呪詛12＋状態5）が実プレイ可能。
`nocturne` / `random-nocturne` を CARD_SET に昇格し、mix-all にも参加（13→14拡張）。
`npm test` **全38スイート緑**（exit 0・整合性4184・不変条件10・nocturne 371件＋nocturne-ui 87件）。
**多エージェント敵対レビュー（6観点）で確定30件・棄却0 を全部修正済み**（うち [high] 2件＝CPU の本番 livelock）。
CPUソーク105戦＝完走105・膠着0・例外0・保存則違反0。`verify:e2e` 9/9（webp 450/0）。
絵（webp）77枚も回収し、**push 済（`643cbd5`・`sw.js` v62）＝本番反映を実機確認した**
（Pages の webp 77枚がバイト一致／Render で夜想曲セットの対戦が正しく開始）。
【次にやること】**発売順の未着手拡張＝同盟／略奪／日の出づる国**（段階1すら未着手）。
その後の未着手拡張＝**同盟／略奪／日の出づる国**（段階1すら未着手）。
詳細と落とし穴は **§0-28**／**§0-27**／**`docs/research/nocturne_rules.md` 冒頭の「実装前に必読」18項目**。**
