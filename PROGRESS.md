...
**A4 の多エージェント敵対レビュー（5観点）で確定12件を全修正**（[high] 3件＝追いはぎ/将軍のリアクション窓に
UI が無く人間が詰む・粉屋/歩哨の「見た」札がオンラインで相手に丸見え・**将軍×玉座の間で CPU 無限ループ**）。
**うち1件は出荷済み暗黒時代の既存バグ**＝青空市場が城塞(fortress)の廃棄に反応しなかった
（`DOM.KINGDOM_DARKAGES` は城塞と青空市場を両方含む＝**今日の本番で踏める**）。**公式へ寄せる方向**。
既存14拡張への退行は**クリーン**（A4 前後を同一シードで完全並走＝263ゲーム・72,608ステップ・差分0）。
**【次にやること】A5＝CARD_SET 昇格**（`DOM.KINGDOM_ALLIES` 固定10種の選定＋`allies`/`random-allies`／
mix-all への参加／UIテスト・CPUソーク）→ **絵（webp）72枚の回収** → PROGRESS 更新 → **ユーザー確認の上で** push。
落とし穴は **§0-29 の A4 節の注意**と **`docs/research/allies_rules.md` 冒頭の「実装前に必読」9項目**。**
...
**新セッションは まず `npm test` を実行し 40スイート・オールグリーン（exit 0・整合性4763件・不変条件12・
同盟429件＋UI231件・夜想曲371件＋UI87件・移動動物園253件＋UI64件・ルネサンス320件＋UI62件・帝国272件＋UI75件・
