...
⑤**A2b**（「カードの種別」を山の一番上で判定する横断修正）まで完了し、**すべて push 済**（`sw.js` **v65**・本番反映を実機確認）。
...
1. **【最優先】同盟（Allies）の段階2＝残りは A3 → A4 → A5**（A1/A2/A2b は完了・push 済＝§0-29）。
   - **A3＝Ally（同盟カード）23種**：`state.ally` の決定は A1 で済んでいる。残るのは
     **盤面への表示**・**好意(`p.favors`)を使う窓**（新 pending は4点セット必須）・**23種の効果**。
     ⚠ **Ally が起こす攻撃は「アタックカードのプレイ」ではない＝堀で防げない**（`ATTACKS` に登録しない）。
     ⚠ **獲得したばかりの好意はその場の窓で即使える**（窓を開く前の値をスナップショットしない）。
   - **A4＝王国カード49種**（素直な系→持続→アタック→複雑）。着手前に §0-29 の A2/A2b 節の「注意」を読む。
