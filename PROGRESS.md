...
2. **A1＝好意トークン**（`p.favors`＋付与＋UI バッジ）／**A2＝分割山6組**（混合山モデル＋循環）。
3. **A3＝Ally 23種**（`createInitialState` で1枚決定＋盤面表示＋好意の使用窓）。
4. **A4＝王国カード49種**（素直な系→持続→アタック→複雑）。
5. **A5＝CARD_SET 昇格** → 多エージェント敵対レビュー → UIテスト新設・CPUソーク・`verify:e2e`/`verify:visual`。
6. **絵（webp）の回収 72枚**（分割山のプレースホルダ6枚は最安カードの絵を流用＝城 `castles` と同じやり方）
