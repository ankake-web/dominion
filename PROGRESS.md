...
### 【次にやること】
1. **絵（webp）77枚の回収**（現在は枠＋文字だけ＝46〜66KB。記憶 `chatgpt-card-art-workflow` の手順）。
2. PROGRESS 更新 → **ユーザー確認の上で** push（`sw.js` v61）。
3. その後の未着手拡張（段階1すら未着手）：**同盟／略奪／日の出づる国**。
...
【次にやること】**絵（webp）77枚の回収** → PROGRESS 更新 → **ユーザー確認の上で** push。
...
- **2026-08-12 時点＝全598枚（縦型450＋横型148）が実プレイ可能**（絵は夜想曲77枚だけ未回収）。
...
- **2026-08-12 に夜想曲（Nocturne・77種）が実プレイ可能になった**（§0-28。絵は未回収＝枠＋文字のみ）。
...
1. **【最優先】夜想曲の絵（webp）77枚の回収** → PROGRESS 更新 → **ユーザー確認の上で** push（`sw.js` v61）。
   段階2は §0-28 で完成済み（全77種が実プレイ可能・全38スイート緑）。
