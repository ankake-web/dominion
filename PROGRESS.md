...
1. **【最優先】略奪（Plunder・2022年12月）＝段階0（公式ルール研究）から**。§4 のユーザー決定Cにより、
   残る2拡張を作り切って**全拡張制覇**にする。**略奪 → 日の出づる国** の発売順。
   ### 規模（英語wiki の `Plunder_(expansion)` で確認済み・2026-08-13）
   **500枚＝王国400（40種）＋ランダマイザー40＋Loot 30枚（15種×2）＋特性(Trait)15＋イベント15 ＝ 実装対象85種**。
   新機構＝**Loot（戦利品の非サプライ山）／特性(Trait)＝横型でサプライの山1つに付ける／"Next time"型の持続**。
   ### 進め方（同盟 §0-29 と同じ4段階。各段階の末で `npm test` 全緑を確認してコミット）
   1. **段階0＝公式ルール研究**（多エージェント収集 → **別エージェントが一次資料で敵対検証**）
      → `docs/research/plunder_rules.md` に統合し、**冒頭に「実装前に必読」**を作る。
      取り方はメモリ `dominion-rules-research-method`（英語wikiは `python tools/wikifetch.py` で Wayback 経由・
      **snapshot の年が2022以前なら略奪の情報は無い**／日本語名は wikiwiki.jp/dominiondeck が正本／
      RGG の PDF は一般ルールの逐語にだけ使う）。
      ⚠ **2026-08-13 のセッションでこの研究WFを1本走らせた**（7群＝王国3分割＋Loot＋イベント＋特性＋機構、
      各群を別エージェントが敵対検証）。**中間成果は `C:/tmp/plunder_research/*.md`**。
      完走していれば流用してよい。無ければ**やり直す**（べき論としては同じWFをもう一度走らせるだけ）。
   2. **段階1＝カタログと画像**：`DOM.CARDS`（王国40＋Loot 15）／`DOM.LANDSCAPES`（イベント15＋特性15）＋
      孤立プール＋`GAIN_ORDER`＋カード一覧の群＋webp（枠＋文字。絵は最後に回収）。
      ⚠ **孤立プールは `DOM.STAGE1_POOLS` に必ず入れる**（入れないと闇市場に「買っても何も起きない死に札」が並ぶ）。
      ⚠ **特性(Trait)は横型の新 kind**＝`build-landscape.js` に新スキンが要る（Ally の `ally` スキンが直近の前例）。
   3. **段階2＝engine/CPU/UI**：新機構ごとにバッチを割り、各バッチ末で `npm test` 全緑＋CPUソーク。
