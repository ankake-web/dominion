...
最終更新: 2026-07-01 / branch `main`。**海辺の簡略化2点（封鎖の堀免疫窓・海賊の財宝獲得リアクション）を本格実装＋混成王国のCPU購入バランス調整＝コミット/デプロイ待ち**。
公開: GitHub Pages https://ankake-web.github.io/dominion/ （クライアント）＋ Render（オンライン対戦サーバ）。
**新セッションは まず `npm test` を実行し 1856件オールグリーンを確認**してから着手すること（17スイート）。**未コミット**（作業ツリーに変更あり：`git status` で確認）。

### 海辺の簡略化2点を本格実装＋混成王国CPU購入バランス調整（2026-07-01）
- **未コミット/未デプロイ**。`sw.js` v17→**v18**（ui.js にリアクションモーダル2種を追加したため）。テスト **1856件緑**（海辺 85→100・海辺UI 22→24 に増、他は不変）。
