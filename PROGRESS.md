...
### 【次にやること】段階2（この順）
1. **★最初に決める★ 占星術師団(Order of Astrologers)／メイソン団(Order of Masons) の実装方式**。
   両者は「**シャッフルのたびに好意を払って全カードを見て、払うたびに選び直せる**」対話を要求するが、
   `reshuffleDeck(p)` は**同期・非対話**（74箇所から呼ばれる）。星図・へそくりが同じ理由で自動選択の
   許容簡略化になっている（§0-22）。**23分の2＝約9%のゲームで必ず出る。**
   候補＝(a)常設方針＋自動選択／(b)`reshuffleDeck` の非同期化（横断リファクタ）／
   **(d)ハイブリッド＝片付けの先引きだけ本物の対話にし、効果の途中のシャッフルは常設方針**（有力）。
   ※(b)(d) を採るなら「密使・下役は**好意をくれる前に**シャッフルを起こしうる＝まだ持っていない好意は使えない」
   という**評価順序**を守ること（公式FAQ）。
