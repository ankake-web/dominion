...
## 0-21. 横型ランドスケープ 第3弾＝**冒険イベント20種**（AE0〜AE4 全完了・**未push**・2026-07-12）
...
1. **【最優先】§0-21＝冒険イベント20種の push（ユーザー確認の上で）**。実装・テスト・敵対レビュー・webp は全完了（未push・`sw.js` v48）。push すれば本番 Pages/Render に「冒険＋イベント」セットが出る。
   - **✅段階2 完了済み拡張**：収穫祭(§0-2)／ギルド(§0-4)／異郷(§0-5)／新プロモ(§0-7)／暗黒時代 全56枚(§0-8)／冒険 全38枚(§0-9)／**帝国 縦型36枚(§0-10〜0-16)**＋**E8(§0-17)**＋**帝国ランドマーク21種＋絵(§0-19)**＋**帝国イベント13種(§0-20)**（すべて push 済）＋**冒険イベント20種(§0-21・未push)**。
2. **段階2の残り＝発売順の未着手拡張**（着手前に `docs/adding-cards.md` を必読）:
   - **発売順その先（段階1すら未着手＝画像・カタログとも無し）**：夜想曲/ルネサンス/移動動物園/同盟/略奪/日の出づる国。
   - 冒険イベント20種の**絵（webp）回収**（現在は枠＋文字のみ＝`tools/build-landscape.js` の event スキン）。
