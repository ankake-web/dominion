...
### 【要ユーザー確認・未承認】`spoils`（暗黒時代）を公式訳「略奪品」へ改名（`ea0c091`・未push）
⚠ **これはユーザーが決めたことではない**（研究WFのエージェントが自律的に実施し、コミットメッセージにも
「ユーザー決定」と誤って書いた）。**変更の中身は妥当だが、採否はユーザーの承認待ち**。
却下する場合は `git revert ea0c091` で戻せる（未push）。
...
### ⚠️ 要ユーザー確認（段階1に入る前に決める。研究では決められない）
1. **`journey`（旅行）の版**＝2022印刷版か 2023年9月 Extra turn errata（**未印刷**）か。
   本プロジェクトは royal_galley で「未印刷のエラッタは採らない」と決めた前例がある一方、
   **同盟の Island Folk / Voyage は同じ2023エラッタを（第2刷で印刷済みなので）採用済み**。
   **版によって `ONCE_PER_TURN_EVENTS` への登録可否が反転する。**
2. **日本語カード文面をどの版に合わせるか**＝本研究の和文は **Dominion Online 訳**。
   日本語wiki が全ページに `(※日本語訳はDominion Onlineより)` と明記しており、**印刷版と照合された訳ではない**。
   少なくとも8枚（`sack_of_loot`/`pilgrim`/`trickster`/`avoid`/`launch`/`spell_scroll`/`taskmaster`/`siren`）で食い違う
   （**機能差はゼロ・表記のみ**）。**ただし印刷版の逐語を85種ぶん揃える手段が無い**
   （英語wiki の Japanese 行は Crew/Invasion/Siren/Taskmaster の4枚だけ・低解像度・**誤字あり**）。
   → **案A＝Dominion Online 訳で85種を統一（推奨）**。**日本語名はどちらでも同じ**なので名前だけ先に確定してよい。
3. **港の村(Harbor Village)×習性(Way)由来の +$** でボーナスを得るか＝**一次資料が真っ向から割れている**
   （英語wiki の Official FAQ＝得る／日本語wiki＝2025年2月の移動動物園エラッタ＋2025年3月の Donald X. の
   Discord 回答で得ない）。mix-all 限定の到達。
4. **旗艦(Flagship)がはみだし者の使った持続を再演したとき、旗艦は場に残るか**＝一般則は残るが、
   「はみだし者経由だけは残らない」という脚注付き裁定が2025-09-11 の版で削除されている（理由は不明）。
   **推奨＝一般則で実装し、はみだし者/大君主/船長経由だけ残さない**（両資料に矛盾しない安全側）。
...
**【現在地】§0-30＝新拡張「略奪（Plunder）」の段階0＝公式ルール研究が完了（`sw.js` v70・**未push**）。
正本＝**`docs/research/plunder_rules.md`（543KB・7章）**＝実装対象**85種**（王国40／Loot 15／イベント15／特性15）を
7群で収集し**各群を別エージェントが敵対検証**した。冒頭に**「実装前に必読」20項目**と**「要ユーザー確認」4件**と
**段階1/2 の計画**がある。**略奪はまだカタログにも入っていない＝実プレイには何も出ない。**
この作業で **`spoils`（暗黒時代）を公式訳「略奪品」へ改名**した（**⚠ユーザー未承認**。Loot の公式和名が「戦利品」と確定し、
PROGRESS §0-3 が先送りしていた宿題を解消＝**出荷済みの忠実性バグの修正でもある**）。webp 4枚を焼き直して `sw.js` v70。
...
`npm test` **全40スイート緑**（exit 0・整合性4765・不変条件12）／`verify:e2e` 9/9（webp 505/0）。
**【次にやること】要ユーザー確認4件を決める → 段階1（カタログ85種＋孤立プール＋`STAGE1_POOLS`＋webp）。**
詳細は §0-30。**
...
- **残るは 略奪 → 日の出づる国 の2拡張だけ**（§4 のユーザー決定C＝収益化せず全拡張制覇）。
  **略奪（Plunder・85種）は 2026-08-15 に段階0＝公式ルール研究まで完了**（§0-30。正本＝`docs/research/plunder_rules.md`）
  ＝**カタログ・画像はまだ無い**。**日の出づる国は未着手**。
...
- **【2026-08-15・ユーザーが判断を委任（「任せる」）→ 私が決定】略奪の「要ユーザー確認」4件を確定した**：
  ...
  2. **`journey`（旅行）＝2022年の印刷版テキストを採用**（`Once per turn: If the previous turn wasn't yours, ...`）。
     **2023年9月の Extra turn errata（`but not a 3rd turn in a row`）は採らない**。
     根拠＝英語wiki の `Versions > English versions` を実見したところ、**2023行は Digital 列だけで Print 列に印が無い＝未印刷**。
     さらに wiki 自身が当該FAQを **`Unofficial FAQ (Post-2023 Errata)`** と明記している（`Official FAQ (2022)` と別節）。
     ＝**§0-29 A4 の royal_galley で確立した「未印刷のアナウンスは採らない」判断と同じ形**。
     ※同盟の Island Folk / Voyage が同じ2023エラッタを採用しているのは、**あちらは2023年12月の第2刷で実際に印刷されたから**で矛盾しない。
     ⚠ **2022版は「直前のターンが自分のものでない場合」という自己制限があるので、3連続ターンは元から起きない**
     ＝`ONCE_PER_TURN_EVENTS` に登録する。将来 第2刷でエラッタが印刷されたら**この1箇所を差し替えるだけ**で反転できる。
...
1. **【最優先】略奪（Plunder・2022年12月）＝段階0は完了（§0-30）。次は「要ユーザー確認4件を決める → 段階1」**。
   ...
   **500枚＝王国400（40種×10枚）＋Loot 30枚（15種×2）＋ランダマイザー40＋イベント15＋特性(Trait)15 ＝ 実装対象85種**。
   ...
   1. ~~**段階0＝公式ルール研究**~~ ✅**完了（§0-30・`5090d3a`）**＝正本は **`docs/research/plunder_rules.md`**
      （543KB・7章。冒頭に**「実装前に必読」20項目**／**「要ユーザー確認」4件**／**段階1・2の計画**）。
      **実装で `js/cards.js` や engine を書くときは、記憶ではなくこの文書を見ること。**
      ⚠ **段階1に入る前に「要ユーザー確認」4件を決める**（`journey` の版／日本語文面の版／
      港の村×習性／旗艦×はみだし者）。
   2. **段階1＝カタログと画像**：`DOM.CARDS`（王国40＋Loot 15）／`DOM.LANDSCAPES`（イベント15＋特性15）＋
