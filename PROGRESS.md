...
## 段階2 **A5＝CARD_SET 昇格＝同盟(Allies)の実プレイ化 完了**（2026-08-13・`sw.js` v68・未push）
...
**【現在地】§0-29＝新拡張「同盟（Allies）」＝**A5（CARD_SET 昇格）まで完了＝実プレイ可能になった**（`sw.js` v68・**未push**）。
...
1. **【最優先】同盟（Allies）の段階2＝残りは「絵72枚の回収」だけ**（A1/A2/A2b/A3/A4 は push 済／**A5 は未push**＝§0-29）。
   - ~~**A3＝Ally（同盟カード）23種**~~ ✅完了（盤面表示＋好意の使用窓＋全効果＋敵対レビュー7件の修正）。
   - ~~**A4＝王国カード49種**~~ ✅完了（5バッチ＋多エージェント敵対レビュー確定12件の修正）。詳細は §0-29 の A4 節。
   - ~~**A5＝CARD_SET 昇格**~~ ✅**完了（2026-08-13・`sw.js` v68・未push）**＝`allies`（固定10種）／`random-allies`／
...
2. **絵（webp）の回収＝同盟の72枚だけが残っている**（王国49＋Ally 23。**分割山のプレースホルダ6枚は
   ...
   - **✅段階2 完了済み拡張**：収穫祭(§0-2)／ギルド(§0-4)／異郷(§0-5)／新プロモ(§0-7)／暗黒時代 全56枚(§0-8)／
     ...
     ＝**すべて push 済**。**同盟 全72種(§0-29)＝A5 まで完了・未push**。
