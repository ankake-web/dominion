...
1. **【最優先】§0-20＝帝国イベント13種の EV2→EV3（横型ランドスケープ第2弾）**。EV0（基盤 `BUY_EVENT`）＋EV1（簡単10種）は完了・ローカルcommit済（`8caabc0`・未push・全35スイート緑）。正本＝`docs/research/landscape_cards.md` §2/§3/§4＋`landscape_gaps.md`。§0-20 の「次にやること」に手順あり。
   - **EV2＝重量イベント3種**：tax（新 `state.pileDebt`＝全山準備+1・購入フェイズの獲得で山の負債を受取・自身は山選択で+2）／donate（次ターン開始時にデッキ全掃討→任意廃棄→5枚引く＝`DURATION_RESOLVERS`/`startQueue`・**タイミングがエラッタの主因＝繊細**）／annex（捨て札5枚残し他を `reshuffleDeck` で山札へ＋公領・負債8）。各4点セット＋終端保証。
   - **EV3＝CARD_SET昇格**：`empires-events`（帝国固定10＋`eventsFrom:'empires'` で2抽選）＋UI picker＋**CPUのイベント購入評価（bestEventBuy・現状CPUはイベントを買わない）**＋CPUソーク＋敵対レビュー（多エージェント・node再現）→ 任意で event webp（build-landscape.js は event スキン対応済）→ **ユーザー確認の上で push**。
   - **✅段階2 完了済み拡張**：収穫祭(§0-2)／ギルド(§0-4)／異郷(§0-5)／新プロモ(§0-7)／暗黒時代 全56枚(§0-8)／冒険 全38枚(§0-9)／**帝国 縦型36枚(§0-10〜0-16)**＋**E8(§0-17)**＋**帝国ランドマーク21種＋絵(§0-19・push済 v46)**（すべて push 済）＋**帝国イベント EV0/EV1(§0-20・未push)**。
2. **段階2の残り＝発売順の未着手拡張**（着手前に `docs/adding-cards.md` を必読）:
   - **冒険のイベント20**＝帝国イベント完了後（横型枠は §0-18 対応済・カタログ研究は `docs/research/landscape_cards.md`。トークン中心・負債なし＝相性は別）。
