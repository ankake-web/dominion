...
**【現在地】§0-30＝新拡張「略奪（Plunder）」の**段階0（公式ルール研究）＋段階1（カタログ85種＋webp）が完了**
（`sw.js` v72）。正本＝**`docs/research/plunder_rules.md`**（7章）＝実装対象**85種**（王国40／Loot 15／
...
`npm test` **全40スイート緑**（exit 0・整合性**5303**・不変条件12）／`verify:e2e` 9/9（webp 560/0）。
⚠ **この日は2つのセッションが並行して同じリポジトリを触り、矛盾するコミットが出た**（§0-30 の「経緯」節）。
**【次にやること】段階2＝P1（Loot の山の基盤）から**。バッチ順は正本の §「段階2 の推奨バッチ順」。**
...
1. **【最優先】略奪（Plunder・2022年12月）＝段階0・段階1とも完了（§0-30）。次は段階2＝P1（Loot の山の基盤）**。
