...
最終更新: 2026-07-10 / branch `main`（最新は `git log` で確認）。**帝国（Empires）縦型36枚は実プレイ可能（§0-16）＋E8＝命令(Command)の忠実化も完了・push済（`52bba46`・§0-17）＝本番反映を実機確認済み**（`sw.js` v44）。**いまは §0-18＝横型ランドスケープ（第1弾＝帝国ランドマーク21種）に着手中**（横型枠パイプライン・研究・カタログ21枚まで完了。engine はこれから）。以後の拡張も 完成→CARD_SET昇格→全テスト緑→**都度ユーザー確認の上で** push（勝手に push しない）。
