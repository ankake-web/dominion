...
最終更新: 2026-07-10 / branch `main`（最新は `git log` で確認）。**帝国（Empires）は E7＝CARD_SET昇格まで完了＝縦型36枚すべて実プレイ可能**（§0-16。`sw.js` v43）。**未pushコミットが4件ある**（E6本体＋handoff2件＋E7。`git log --oneline origin/main..HEAD` で確認）。**push すれば本番に「帝国セット」「帝国から」が出る**＋E6のティアラ/偽造通貨/はみだし者の実バグ修正＋**E7で見つけたUIの実バグ2件（拡張の固定セット8つが画面から選べない／ランダム分類のボタンが画面外で押せない）の修正**も反映される。以後の拡張も 完成→CARD_SET昇格→全テスト緑→**都度ユーザー確認の上で** push（勝手に push しない）。
...
## 0-16. 段階2＝帝国（Empires）E7＝Phase E＝CARD_SET昇格 **完了**（2026-07-10・WIP・未push）
...
### 【次にやること】push（ユーザー確認）→ その後 E8 or 横型ランドスケープ
- **push すると本番に帝国が出る**（＋上記の実バグ修正が全部効く）。`sw.js` v43。
...
1. **【最優先】push（ユーザー確認）＝未pushコミット4件（§0-16）**。push すると本番に「帝国セット」「帝国から」が出て、E6の実バグ修正3件＋E7の実バグ修正5件（大君主×負債／拡張セットが選べない／ランダム分類が押せない／CPUの混合山コスト＆終局読み／castles.webp 404）も反映される（`sw.js` v43）。
   - **✅段階2 完了済み拡張**：収穫祭(§0-2)／ギルド(§0-4)／異郷(§0-5)／新プロモ(§0-7)／暗黒時代 全56枚(§0-8)／冒険 全38枚(§0-9)／**帝国 縦型36枚(§0-10〜0-16・CARD_SET昇格まで完了)**。基本・陰謀・海辺・錬金術・繁栄と合わせ、**縦型カードの実プレイ化は帝国まで完了**。
   - **次の候補＝E8（任意・忠実性）＝命令(Command)の「自身が動く」clause**（§0-16 末尾）。overlord/band_of_misfits/captain/prince の4枚が対象。
