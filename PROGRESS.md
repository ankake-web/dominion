...
同盟310件＋UI99件・夜想曲371件＋UI87件・移動動物園253件＋UI64件・ルネサンス320件＋UI62件・帝国272件＋UI75件・
...
1. **【最優先】同盟（Allies）の段階2＝残りは A4 → A5**（A1/A2/A2b は push 済／**A3 は完了・未push**＝§0-29）。
   - ~~**A3＝Ally（同盟カード）23種**~~ ✅完了（盤面表示＋好意の使用窓＋全効果＋敵対レビュー7件の修正）。
   - **A4＝王国カード49種**（素直な系→持続→アタック→複雑）。着手前に §0-29 の A2/A2b/A3 節の「注意」を読む。
     ⚠ **Ally が起こす攻撃は「アタックカードのプレイ」ではない＝堀で防げない**（`ATTACKS` に登録しない）。
     ⚠ **獲得したばかりの好意はその場の窓で即使える**（窓を開く前の値をスナップショットしない）。
     ⚠ **連携(Liaison)が好意をくれるのは A4**（今は開始時の1個／輸入者の5個しか無い）。
     ⚠ **新しく「カードを使用する」経路を足したら `noteAllyPlay` を呼ぶ**（A3 の最大の取りこぼしだった）。
   - **A5＝CARD_SET 昇格**（`DOM.STAGE1_POOLS` から `'allies'` を外す）→ 多エージェント敵対レビュー →
     UIテスト新設・CPUソーク・`verify:e2e`/`verify:visual` → **絵（webp）72枚の回収** → push。
