...
- **npm test 全38スイート緑（exit 0・整合性4184・不変条件10）**／CPU序列 100/64/95 維持。
  新設2スイート＝`test/nocturne.test.js` **324件**／`test/nocturne-ui.test.js` **70件**
  （全35 pending に「押せる選択肢が1つ以上ある」ことを jsdom で検査＝人間が詰まないことの担保）。
- **CPUソーク72戦**（nocturne／random-nocturne／mix:nocturne,basic／mix:nocturne,menagerie,empires ×2〜4人×
  全難易度）＝完走72・膠着0・例外0・保存則違反0。
...
`npm test` **全38スイート緑**（exit 0・整合性4184・不変条件10・nocturne 324件＋nocturne-ui 70件）。
CPUソーク72戦＝完走72・膠着0・例外0・保存則違反0。`verify:e2e` 9/9（webp 450/0）。
