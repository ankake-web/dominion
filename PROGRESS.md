...
**§4 のユーザー決定C（収益化せず全拡張制覇）により、残る未着手は 略奪 → 日の出づる国 の2拡張だけ**。その1本目に着手。
...
### 【次にやること】
- **push（ユーザー確認）**＝押せば GitHub Pages（クライアント）と Render（サーバ）に略奪が出る。
- その後は **日の出づる国（Rising Sun）＝最後の1拡張**。
...
**【次にやること】push（ユーザー確認）→ その後は 日の出づる国（Rising Sun）＝最後の1拡張。**
...
このまま全拡張を作り切る**により、残るは **略奪 → 日の出づる国** の2拡張だけ。
...
- **残るは 略奪 → 日の出づる国 の2拡張だけ**（§4 のユーザー決定C＝収益化せず全拡張制覇）。
  **略奪（Plunder・85種）は 2026-08-15 に段階0（研究）＋段階1（カタログ85種＋webp）まで完了**
  ...
  （`DOM.CARDS` 560／`DOM.LANDSCAPES` 201＝計761枚。**絵は未回収＝枠＋文字だけ**）。**日の出づる国は未着手**。
...
- **【2026-08-15・ユーザーが判断を委任（「任せる」）→ 私が決定】略奪の「要ユーザー確認」4件を確定した**：
  ...
  3. **日本語のカード文面＝Dominion Online 訳で統一**（日本語wiki が出典として明記しているもの）。
     ...
     **【2026-08-15・ユーザーに確認済み】ユーザーは日本語版の現物を持っていない**＝印刷版と照合する手段は
     このプロジェクトには無い。**この件は決着済みなので蒸し返さないこと**（日の出づる国でも同じ扱いでよい）。
...
- **【2026-08-15・ユーザー決定】このアプリは「趣味として全拡張制覇」まで作り切る（収益化はしない）**。
  ...
  **C（収益化はせずこのまま全拡張を完成させる）** を提示し、**ユーザーはCを選択**。
  ＝**残るは略奪（Plunder）と日の出づる国（Rising Sun）の2拡張**。収益化の話は当面持ち出さない。
...
1. **【最優先】略奪（Plunder・2022年12月）＝段階0・段階1・段階2 P1（戦利品の山＋15種）まで完了（§0-30）。次は P2＝"next time" 型持続の共通機構**。
   §4 のユーザー決定Cにより、残る2拡張を作り切って**全拡張制覇**にする。**略奪 → 日の出づる国** の発売順。
...
2. **日の出づる国（Rising Sun・2023）**＝略奪の後。段階1すら未着手。新機構＝**予言(Prophecy)／日の出トークン／
