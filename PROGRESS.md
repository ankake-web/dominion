...
## 段階2 **A3＝同盟(Ally)カード23種＝完了**（2026-08-12・未push・`sw.js` v66）
...
⑥**A3＝同盟(Ally)カード23種**（盤面表示＋好意の使用窓＋全効果）を実装し、敵対レビュー確定7件も修正済み
（**未push**・`sw.js` **v66**）。うち **[high] 1件は A3 以前からあった UI の穴**（選択のターン跨ぎ持ち越しで人間が詰む）
＝**地下貯蔵庫/民兵/拷問人など既存カードにも効く修正**なので、A4 の完成を待たず push してよい（メモリ push-bugfixes-early）。
`npm test` **全40スイート緑**（整合性4763・不変条件12・**同盟310件**＋**同盟UI 99件**）／`verify:e2e` 9/9（webp 505/0）／
`verify:visual` はみ出し0。**次にやること＝A4＝王国カード49種**（詳細は下の「段階2 A3」節と「注意（A3 固有）」）。
