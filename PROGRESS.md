...
**【現在地】§0-27＝新拡張「夜想曲（Nocturne）」に着手中（未push・`sw.js` v59）。
研究（`docs/research/nocturne_rules.md` 49万字）＋段階1（カタログ77種＋webp）＋段階2-N0（夜フェイズと基盤）まで完了。
`npm test` **全36スイート緑**（整合性4182・不変条件10）。**まだ CARD_SETS 未参照＝実プレイには出ない**。
【次にやること】N0b＝UI の夜フェイズ操作 → N1＝祝福/呪詛/状態 → N2〜N4＝カード効果 → N5＝CARD_SET昇格＋敵対レビュー。
詳細と落とし穴は **§0-27** と **`docs/research/nocturne_rules.md` 冒頭の「実装前に必読」18項目**に全部書いてある。**
...
- **2026-08-11 時点＝全521枚（縦型402＋横型119）が実プレイ可能で、全部に絵が入っている**。
  実装済み拡張＝基本・陰謀（第二版）／プロモ・新プロモ／海辺／錬金術（支配含む）／繁栄／収穫祭／ギルド／
  異郷／暗黒時代／冒険（＋イベント20）／帝国（＋ランドマーク21・イベント13）／ルネサンス（＋プロジェクト20・
  アーティファクト5）／移動動物園（＋習性20・イベント20）。さらに **mix-all**（13拡張を自由に混ぜる）。
- **2026-08-11 に夜想曲（Nocturne・77種）へ着手**＝カタログと webp は入ったが**まだ実プレイ不可**（§0-27）。
...
1. **【最優先】夜想曲（Nocturne）の段階2を完成させる**＝§0-27 の「次にやること」を上から順に
   （N0b＝UI の夜フェイズ操作 → N1＝祝福/呪詛/状態 → N2〜N4＝カード効果 → N5＝CARD_SET昇格＋敵対レビュー → 絵の回収 → push）。
   **実装前に `docs/research/nocturne_rules.md` 冒頭の「実装前に必読」18項目**と `docs/adding-cards.md` を読むこと。
