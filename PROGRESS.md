...
`npm test` **全42スイート緑**（exit 0・整合性**5303**・不変条件12・略奪29＋略奪UI 9）／`verify:e2e` 9/9（webp 560/0）。
⚠ **この日は2つのセッションが並行して同じリポジトリを触り、矛盾するコミットが出た**（§0-30 の「経緯」節）。
**【次にやること】段階2 P1b＝戦利品(Loot) 15種の効果**（`gainLoot` はできたがカードの効果はまだ空）。
一覧と難度は §「段階2 P1a」末尾の表。その後 P2（"next time" 持続の共通機構）→ P3〜P7。**
...
1. **【最優先】略奪（Plunder・2022年12月）＝段階0・段階1・段階2 P1a まで完了（§0-30）。次は P1b＝戦利品15種の効果**。
