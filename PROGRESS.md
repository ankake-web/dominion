...
1. **追放（Exile）＝`p.exile`**：islandMat 型の**公開ゾーン**（allCards／invariants の ZONES／得点に数える）。
   **「サプライから追放」は獲得ではない**（on-gain は誘発しないが山は減る＝3山終了に影響）／追放マットから捨てるのは
   本物の「捨てる」（on-discard が誘発）／**一般ルール＝同名を獲得したとき追放マットの同名を好きな枚数 捨て札に戻してよい**（`exile_discard`）。
...
### 【未修正・発見した既存バグ3件】2025年2月エラッタの波及（Menagerie とは独立）
...
### 【次にやること】
1. **M4＝イベント20種**（`applyEventEffect` に case を足す。BUY_EVENT 基盤は完成済み）。
...
**【現在地】§0-25＝新拡張「移動動物園（Menagerie）」を実装中。王国30＋馬＋習性20＋CARD_SET昇格まで完了、
**イベント20種だけ未実装**（カタログと webp はある＝段階1）。`sw.js` v57・**未push**。
`npm test` 全35スイート緑（exit 0／整合性3694・不変条件9・menagerie 126）。
【次にやること】①イベント20種（`applyEventEffect` に case を足す）→ ②裁定で判明した**既存バグ3件**の修正
（戦車競走が `draw()` を通らず -1カードトークンを無視／使節団の1ターン1回/儀式のコスト参照）→ ③多エージェント敵対レビュー
→ ④`menagerie-ui.test.js`＋CPUソーク＋`verify:e2e`/`visual` → ⑤**絵71枚の回収** → ⑥ユーザー確認の上で push。
詳細と落とし穴は **§0-25** に全部書いてある（着手前に必ず読む）。**
...
1. **【最優先】移動動物園（Menagerie）を完成させる**（§0-25 が正本。着手前に必ず §0-25 を読む）。この順で:
   1. **イベント20種**＝`applyEventEffect` に case を足す（`BUY_EVENT` 基盤は帝国/冒険で完成済み）。
      **絶望(desperation) は `ONCE_PER_TURN_EVENTS` に足す**（"Once per turn:" は**購入自体が1ターン1回**＝一次資料で確定）／
      **好機(seize_the_day) は1ゲーム1回**（`p.seizedTheDay`・相続と同型）。pending が要るのは
      banish/bargain/delay/demand/desperation/enhance/gamble/invest/march/pursue/toil/transport 前後。
      実装後は `menagerie-events` セット（固定10＋イベント2枚抽選）を CARD_SETS に足す。
   2. **裁定で判明した既存バグ3件**（§0-25「未修正・発見した既存バグ3件」）。
      特に**戦車競走が `draw()` を通っておらず -1カードトークンを無視する**のは実挙動バグ（mix-all で到達可能）＝回帰テスト必須。
   3. **多エージェント敵対レビュー**（観点＝追放の忠実性／馬の非サプライ4系統／習性×既存機構／獲得時対話の競合／
      CPU非ループ／既存拡張への退行）。各 finding は必ず node で再現してから直す。
   4. `menagerie-ui.test.js` 新設（全 pending のモーダル＋追放マット表示＋習性帯＋セット選択）／CPUソーク／
      `npm run verify:e2e`（webp 404 検査＝71枚増えている）／`verify:visual`。
   5. **絵（webp）71枚の回収**（現在は枠＋文字の暗い板）。ChatGPT で生成 → `asset/art/<id>.png` → 再生成。
      記憶 `chatgpt-card-art-workflow` の手順どおり（縦型31＝`build-cards.js`／横型40＝`build-landscape.js`）。
   6. PROGRESS 更新 → **ユーザー確認の上で** push（`sw.js` v57）。
2. **その次の拡張候補**（着手前に `docs/adding-cards.md` 必読）:
