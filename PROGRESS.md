...
### 検証
- **npm test 全42スイート緑（exit 0・整合性 5303→5307・不変条件12）**／
  `test/plunder.test.js` 136→**455件**（P2〜P7 の裁定＋各バッチのCPUソーク＋出荷4セット8戦＋mix混成3戦）／
  `test/plunder-ui.test.js` 37→**148件**（全 pending にモーダルと押せる選択肢があるか・CPU が null を返さないか）。
- `verify:e2e` 9/9（webp 560/0）／**略奪の webp 85/85 が存在**することを機械検算。

### 【次にやること】
1. **多エージェント敵対レビュー（4観点）を実施中** → 確定した問題を修正。
2. **絵（webp）85枚の回収**（現在は枠＋文字だけ）＝ChatGPT で生成 → `asset/art/<id>.png` → 再生成（このPCのみ）。
3. PROGRESS 更新 → **ユーザー確認の上で** push。
