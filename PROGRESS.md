...
### 【次にやること】
0. **敵対レビューの確定10件は修正済み（`sw.js` v54）＝push 済み**。
1. **帝国イベント13種の絵（webp）回収**＝下記の「注意」参照。**現在、絵が無いのはこの13枚だけ**（全450枚を実測）。
   指示文は scratchpad の `empires_events_art_prompt.md`（バッチA=10枚／バッチB=3枚）。
2. **コミット＋push（ユーザー確認の上で）**＝`sw.js` v53。オンラインUndoはサーバ側コード変更があるので Render の再デプロイが要る。

### 注意（次セッションが知らないと事故る）
- **PROGRESS の旧記述「実装済みの全カードに絵が入っている（枠＋文字だけの札はゼロ）」は誤りだった**。
  **帝国イベント13種（advance/annex/banquet/conquest/delve/dominate/donate/ritual/salt_the_earth/tax/triumph/wedding/windfall）は
  今も枠＋文字だけ**（`asset/art/<id>.png` が無く webp も 39〜60KB＝絵入りの 85〜125KB より明らかに小さい）。
  再生成＝`CARDS_ONLY=<ids> node tools/build-landscape.js`（横型・このPCのみ）。
...
【次にやること】① 帝国イベント13種の絵の回収（**絵が無いのはこの13枚だけ**＝下の注意参照）
② ユーザー確認の上でコミット＋push（オンラインUndoはサーバ変更あり＝Render 再デプロイが要る）。
【次の大仕事】発売順の未着手拡張（夜想曲／移動動物園／同盟／略奪／日の出づる国）。**

**【重要な訂正】「実装済みの全カードに絵が入っている」は誤り**＝**帝国イベント13種は今も枠＋文字だけ**
（advance/annex/banquet/conquest/delve/dominate/donate/ritual/salt_the_earth/tax/triumph/wedding/windfall）。全450枚を実測して確認済み。
...
2. **次の拡張候補**（着手前に `docs/adding-cards.md` 必読）:
   - **【未完】帝国イベント13種だけ絵が無い**（枠＋文字の暗い板のまま）。他は全部回収済み
     ＝ルネサンス50枚（2026-07-13）／冒険イベント20種（2026-07-14）／帝国ランドマーク21種（2026-07-11）。
     **全450枚（縦371＋横79）を走査して、`asset/art/<id>.png` が無いのはこの13枚だけと確認済み（2026-08-06）**。
