...
1. **【最優先】次の拡張候補**（着手前に `docs/adding-cards.md` 必読）。§0-21（冒険イベント20種）は **push済・本番反映確認済**。
   - **発売順の未着手拡張（段階1すら未着手＝画像・カタログとも無し）**：夜想曲/ルネサンス/移動動物園/同盟/略奪/日の出づる国。
   - **冒険イベント20種の絵（webp）回収**（現在は枠＋文字のみ。`tools/build-landscape.js` の event スキン＋`asset/art/<id>.png`）。
