...
**【現在地】§0-26＝新拡張「移動動物園（Menagerie）」が完成し、絵まで入れて push 済（`sw.js` v58・本番反映確認済）。
...
- **2026-08-11 時点＝全521枚（縦型402＋横型119）が実プレイ可能で、全部に絵が入っている**。
  ...
  **未着手＝夜想曲／同盟／略奪／日の出づる国**（画像・カタログとも無し）。
...
1. **【最優先】次の拡張に着手する**（着手前に `docs/adding-cards.md` 必読）。
   **発売順の未着手拡張（段階1すら未着手＝画像・カタログとも無し）**：夜想曲／同盟／略奪／日の出づる国。
