...
**【現在地】§0-31＝新拡張「旭日（Rising Sun）」＝段階0（公式ルール研究）完了**（`sw.js` v76・**未pushが10コミット**）。
...
1. **【最優先】出荷済みバグ候補6件を潰す**（§0-31 の「残っているバグ候補」の表）。
   旭日の段階0の研究で出た未検証の候補。**段階1に入る前にここを片づける**（engine を触る前のほうが安全）。
   進め方＝**必ず node で再現してから直す**（エージェントの指摘を鵜呑みにしない）＋
   **回帰テストはバグ注入で感度を確かめる**（今回、感度の無いテストを1件書いてしまった）。
   最有力＝**冠/首謀者/門下生が `canPlayHandCard` を通らない**（`random-allies` で到達）。
