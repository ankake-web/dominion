...
最終更新: 2026-07-13 / branch `main`（最新は `git log` で確認）。**§0-22＝新拡張ルネサンス（Renaissance）全50枚＝王国25＋プロジェクト20＋アーティファクト5 を R1〜R6 で全実装＋敵対レビュー確定9件を修正（未push・`sw.js` v49・新セット `renaissance`／`renaissance-projects`／`random-renaissance`）。既存エンジンに横断リファクタ2件（`reveal()` に公開フック集約＝パトロン／財宝判定を `isTreasureFor` に集約69箇所＝資本主義）。npm test 全37スイート緑（整合性3401・不変条件8）・CPUソーク133戦クリーン・verify:e2e 9/9（webp371）。次＝push（ユーザー確認）。**
（以下は §0-21 までの旧サマリ）
