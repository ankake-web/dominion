...
最終更新: 2026-08-13 / branch `main`（最新は `git log` で確認）。

**【現在地】§0-29＝新拡張「同盟（Allies）」＝**A5（CARD_SET 昇格）まで完了＝実プレイ可能になった**（`sw.js` v68・**push済＝本番反映を実機確認**）。
...
**【次にやること】未着手の拡張＝略奪／日の出づる国**（段階1すら未着手＝画像・カタログとも無し）。
着手前に `docs/adding-cards.md` を読むこと。
...
1. **【完了】同盟（Allies）＝段階2 も絵の回収も完了**（A1〜A5＋絵72枚＝§0-29）。
   - ~~**A3＝Ally（同盟カード）23種**~~ ✅完了（盤面表示＋好意の使用窓＋全効果＋敵対レビュー7件の修正）。
   - ~~**A4＝王国カード49種**~~ ✅完了（5バッチ＋多エージェント敵対レビュー確定12件の修正）。詳細は §0-29 の A4 節。
   - ~~**A5＝CARD_SET 昇格**~~ ✅**完了・push済（2026-08-13・`sw.js` v68・本番反映を実機確認）**＝`allies`（固定10種）／`random-allies`／
     mix-all（14→15拡張）／`STAGE1_POOLS` を空に（闇市場に同盟の非分割25種が入る）／
     CPU の `GAIN_ORDER` を実強度順へ／**多エージェント敵対レビュー確定10件を全修正**。詳細は §0-29 の A5 節。
   - ~~**絵（webp）72枚の回収**~~ ✅完了（2026-08-13・`sw.js` v69・全676枚に絵が入った）。
   - （以下は A5 の手順の記録。完了済み）：
     ① `js/cards.js` の **`DOM.STAGE1_POOLS` から `'allies'` を外す**（＝闇市場デッキにも同盟が入るようになる）。
     ② **`DOM.KINGDOM_ALLIES` 固定10種を選定**（公式の同盟専用10種は無いので自作 showcase。
        **連携を必ず1枚入れる**＝入れないと Ally も好意も登場しない。分割山と非分割を混ぜ、
        循環・持続・アタック・好意の使い道をひと通り味わえる構成にする）＋ `allies` / `random-allies` を `CARD_SETS` に追加。
     ③ `MIX_KINGDOM_POOLS.allies` に追加して mix-all（14→15拡張）へ参加させる。
     ④ `invariants` の出荷セット検証に `allies` / `random-allies` を追加。UI の「拡張」タイルに出ることを確認
        （`test/ui.test.js` の「全 CARD_SETS がセット選択画面から選べる」が守る）。
     ⑤ 昇格後に**もう一度**多エージェント敵対レビュー＋CPUソーク（**闇市場に同盟が入る経路が新たに開く**ので
        分割山の中身が漏れないか＝`MIXED_PILE_CONTENTS` の確認が要る）。
     ⑥ **絵（webp）72枚の回収**（手順はメモリ `chatgpt-card-art-workflow`。分割山のプレースホルダ6枚は
        最安カードの絵を流用してよい＝城 `castles` と同じやり方）→ `sw.js` の VERSION を上げる。
     ⑦ PROGRESS 更新 → **ユーザー確認の上で** push。
   着手前に **`docs/research/allies_rules.md` 冒頭の「実装前に必読」9項目**と **§0-29 の A5／A4 の注意節**を読むこと。
2. **【次にやること】未着手の拡張＝略奪／日の出づる国**（段階1すら未着手＝画像・カタログとも無し）。
   ✅**絵の回収は全拡張で完了**（同盟72枚を 2026-08-13 に回収＝**全676枚に絵が入っている**）。
   手順はメモリ `chatgpt-card-art-workflow`。**⚠ ChatGPT は指示した順番どおりに生成するとは限らない**＝
   連番で機械割当せず**必ず全枚を実見して判別**する（同盟でも K1 が10枚中9枚ズレる状態だった）。
   絵の有無は webp のサイズで見分けられる（枠＋文字だけ＝36〜67KB／絵入り＝73〜207KB）。
   再生成＝縦型 `CARDS_ONLY=<ids> node tools/build-cards.js`／横型 `CARDS_ONLY=<ids> node tools/build-landscape.js`
   （**このPCのみ**＝入力の `images/`・`asset/art/` は gitignore）。回収したら `sw.js` の VERSION を上げる。
   - **未着手の拡張**（段階1すら未着手＝画像・カタログとも無し）：**略奪／日の出づる国**。
     新しいカードを足す前に `docs/adding-cards.md` を読むこと。
   - **✅段階2 完了済み拡張**：収穫祭(§0-2)／ギルド(§0-4)／異郷(§0-5)／新プロモ(§0-7)／暗黒時代 全56枚(§0-8)／
     冒険 全38枚(§0-9)＋イベント20(§0-21)／帝国 縦型36枚(§0-10〜0-16)＋E8(§0-17)＋ランドマーク21(§0-19)＋
     イベント13(§0-20)／ルネサンス全50枚(§0-22)／移動動物園 全71枚(§0-25〜§0-26)／夜想曲 全77種(§0-27〜§0-28)
     ＝**すべて push 済**。**同盟 全72種(§0-29)＝A5 まで完了・push済**。
