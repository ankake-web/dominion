...
**【現在地】§0-23＝mix-all モード（拡張を自由に混ぜるランダム対戦）＝配線＋エンジン硬化（23件）＋敵対レビュー（17件）まで
完了し **push 済（`189a1f4`・`sw.js` v52・本番反映を実機確認）**。全38スイート緑（exit 0・整合性3401・不変条件9・mixall 102）／
mix CPUソーク318戦クリーン／`verify:e2e` 9/9。**未pushコミットは無い**。
＝**実プレイ可能な13拡張を自由に混ぜて遊べる**（横型＝イベント/ランドマーク/プロジェクトも合算2枚まで選べる）。
...
2. **次の拡張候補**（着手前に `docs/adding-cards.md` 必読）:
   - ✅**絵（webp）の回収は完了**：ルネサンス50枚（2026-07-13）／冒険イベント20種（2026-07-14）。
     **現在、実装済みの全カードに絵が入っている**（枠＋文字だけの札はもう無い）。
