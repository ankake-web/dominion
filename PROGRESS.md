...
### 🔴【A4 に入る前に必ずやること】A2b＝「カードの種別」を見る述語を **山の一番上**で判定する横断修正
**敵対レビューが node で実証した未修正の finding（[medium]）**。同盟の分割山は**中身の種別が不均一**なので、
`cardCost` は一番上を見るのに `DOM.isType(山キー, ...)` は randomizer を見る、という**対の不整合**が破れる。
- 実証された誤り3件（いずれも `odysseys`／`forts` を一番上まで掘る／循環すると起きる）：
  - **待ち伏せ(Lurker)** が「サプライのアクションカード」として **沈没した宝物（財宝）** を廃棄できる
  - **昇進(Advance)** が「$6以下のアクションカード」として **沈没した宝物** を獲得できる
  - **塩まき(Salt the Earth)** が「サプライの勝利点カード」として **要塞(Stronghold・勝利点)** を廃棄できない
- **公式の線引き**＝randomizer を見るのは「**山の**コスト・種別」を参照する効果だけ
  （植民／若き魔女の災いカード／冒険の山トークン／発明家の家族／汚された神殿／オベリスク）。
  「アクション**カード**を獲得/廃棄する」は**実カード**（＝今の一番上）を見る。
- **なぜ A2 で直さなかったか**：修正対象は engine 約17箇所＋**CPU と UI の同じ条件の手書き**＝合計40箇所超で、
  §0-23 の `costUpTo` 集約と同規模の横断リファクタになる。**片側だけ直すと engine拒否×CPU提案で本番 livelock**
  （§0-23 で実際に起きた事故）。**同盟はまだ CARD_SETS 未参照＝本番未到達**であり、
  対象カード（沈没した宝物/領地/遠い海岸/要塞）は A4 まで効果ゼロなので、**独立した1コミット＋専用レビュー**で行う。
- **方針の候補**：(a) `isTypeTop(state, id, type)`（＝`mixedTopCard(state,id) || id` の種別）を新設し、
  **カード参照の述語だけ**を engine/CPU/UI 同時に寄せる（`山参照` の述語＝`populatePiles`/`actionSupplyPiles`/
  `pickBane`/`pickMouseCard`/`plainActionPile` はそのまま randomizer）。
  (b) より根治的に、**「混合山の一番上の実カードidを名指しで獲得/廃棄できる」**ように `gainableBase` を広げる
  （下の密輸人・巡礼の簡略化も同時に解消する）。**(a) を先に、(b) は A5 で検討**。
- 起点になる engine の該当行＝`seawayCanGain` / `banquetCanGain` / `advanceCanGain` / `bargainCanGain` /
  `LURKER_CHOOSE`・`LURKER_TRASH` / `anyVictorySupply`・`SALT_TRASH` / 建て直し / 行進 / 大学 / 車大工 /
  従者 / 職人 の各 `DOM.isType(id, ...)`。
...
**【次にやること】① A2b＝「カードの種別」を見る述語を山の一番上で判定する横断修正（A4 の前に必須・§0-29 の A2 節に赤字で記載）
→ ② A3＝Ally 23種 → ③ A4＝王国カード49種 → ④ A5＝CARD_SET 昇格。**
