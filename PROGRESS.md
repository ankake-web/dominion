...
最終更新: 2026-07-01 / branch `main`。**多エージェント監査でカード種別ラベルの誤表示2件を修正（ペテン師/会計士）＋該当webp再生成＝コミット&push待ち**。
公開: GitHub Pages https://ankake-web.github.io/dominion/ （クライアント）＋ Render（オンライン対戦サーバ）。
**新セッションは まず `npm test` を実行し 2399件オールグリーンを確認**してから着手すること（17スイート。整合性テストに種別ラベル網羅チェックを追加し件数増）。
