...
- **絵（webp）72枚は未回収**＝同盟の78枚（縦55＋横23）は「枠＋文字」のまま本番に出ている（遊ぶぶんには支障なし）。
...
＝**同盟72種（Ally 23＋王国49）が実際に遊べる**。残るは**絵（webp）72枚の回収**だけ。
...
**【次にやること】① 絵（webp）72枚の回収**（王国49＋Ally 23。分割山のプレースホルダ6枚は最安カードの絵を流用＝
城 `castles` と同じやり方。手順はメモリ `chatgpt-card-art-workflow`＝**ChatGPT の生成順は指示順と一致しないので
必ず全枚を実見して判別する**）→ **② PROGRESS 更新 → ③ ユーザー確認の上で push**。
...
- **2026-08-13 時点＝全676枚（縦型505＋横型171）が実プレイ可能**。
...
- **2026-08-13 に同盟（Allies・72種）が実プレイ可能になった**（§0-29 の A5）。
  **絵が入っていないのは同盟の78枚だけ**（縦55＋横23＝枠＋文字のみ。他の598枚には絵がある）。
...
1. **【最優先】同盟（Allies）の段階2＝残りは「絵72枚の回収」だけ**（A1〜A5 すべて push 済＝§0-29）。
   ...
   - **【次はここ】絵（webp）72枚の回収** → PROGRESS 更新 → **ユーザー確認の上で** push。
   - （以下は A5 の手順の記録。完了済み）：
...
2. **絵（webp）の回収＝同盟の72枚だけが残っている**（王国49＋Ally 23。**分割山のプレースホルダ6枚は
   その山の最安カードの絵を流用**＝城 `castles` と同じやり方）。**他の598枚には絵が入っている**。
   手順はメモリ `chatgpt-card-art-workflow`。**⚠ ChatGPT は指示した順番どおりに生成するとは限らない**＝
   連番で機械割当せず**必ず全枚を実見して判別**する（移動動物園で10枚中9枚が取り違えになる状態だった）。
   絵の有無は webp のサイズで見分けられる（枠＋文字だけ＝36〜67KB／絵入り＝73〜201KB）。
