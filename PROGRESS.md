...
### 【次にやること】P2＝**"next time" 型持続の共通機構**
該当は**ちょうど7枚**＝檻/調査/秘境の社/豊穣/旗艦/上陸部隊/切り裂き魔。
**相手のターンに条件が満たされ「相手の片付け」で自分の場から捨てられる**（`cleanupAndAdvance` は
`state.turn.active` の場しか片付けない＝新しい経路が要る）／条件を満たさなければ**永久に場に残る**／
**その事象で場に出た1枚自身は誘発しない**／**誘発したら空振りでも消費される**。詳細は正本の「実装前に必読」1。

#### P2 の設計メモ（この作業で下調べした内容＝次セッションが再発見しなくて済むように）
- **器**＝`p.delayedEffects` に `{card, type, nextTime:'<trigger>'}` を積む（`armDuration` の拡張で足りる）。
  誘発点ごとに `fireNextTime(state, trigger, ctx)` を呼び、**全プレイヤーの**予約を走査する。
  ⚠ **走査リストは事象の「前」にスナップショットする**（`that will trigger your other Secluded Shrines,
  but not the one you just played` ＝その事象で場に出た1枚は誘発しない）。
- **⚠ 一番危ないのは「相手の片付けで捨てる」経路**。既存の `cleanupAndAdvance` は
  **`cnt`（残り予約数）で手番プレイヤーの持続だけを仕分けている**。ここを**全プレイヤーの一括掃除**に
  そのまま広げてはいけない＝**冒険の隊商の護衛（相手のターンにプレイして自分の次の片付けまで場に残す
  §0-9 Batch5c の許容簡略化）** や、前哨地/航海の追加ターンが絡む持続が**早すぎるタイミングで捨てられる**恐れがある。
  → **「next-time 予約が消費されたことを印にして、その1枚だけを捨てる」**（全体掃除にしない）のが安全側。
- **誘発点は7種類**＝勝利点の獲得（檻）／**サプライの山が空になった瞬間**（調査）／財宝の獲得（秘境の社）／
  アクションの獲得（豊穣）／**誰かの $5以上の財宝の獲得**（切り裂き魔）／命令でないアクションの使用（旗艦）／
  そのターン最初に使ったカードが財宝（上陸部隊）。
  ⚠ **調査だけは既存に無いフック**（`emptyPileCount` は「今いくつ空か」であって「今空になった」ではない）。
  ⚠ **切り裂き魔は `anyone gains`**＝自分の獲得でも誘発する／**予約を張るのはアタックを全部解決した後**。
- **既存の同型**＝`applyLingerOnBuy`（沼の妖婆／呪いの森／門番＝相手の購入をフックする持続・§0-9 Batch5c）。
  **一意 `rid` で「この予約1つだけ」を消費する**設計もそこにある（免疫の記録で使っている）。
- **回帰の担保**＝`cleanupAndAdvance` を触るので、**既存14拡張への退行を新旧並走で確認する**こと
  （§0-29 A4/A5 でやった「同一シードで engine を完全並走させ毎ステップ state を突き合わせる」ハーネス）。
...
この作業で **`spoils`（暗黒時代）を公式訳「略奪品」へ改名**した（ユーザー決定＝Loot の公式和名が「戦利品」と
確定したため。**出荷済みの忠実性バグの修正でもある**）。
**版の選択4件はすべて決着**（§4）＝旅行は2023エラッタ／日本語文面は Dominion Online 訳／
港の村×習性は +$ ボーナスなし／旗艦は持続を再演したら場に残る（例外なし）。
一次資料ツールを2本常設化＝**`tools/wikidirect.js`（英語wiki を Anubis 突破で直読み・1ページ数秒・常に現行版）**と
**`tools/jpwiki.py`（日本語wiki）**。**`tools/wikifetch.py`（Wayback）は予備に格下げ。**
`npm test` **全42スイート緑**（exit 0・整合性**5303**・不変条件12・略奪**136**＋略奪UI **37**）／`verify:e2e` 9/9（webp 560/0）。
⚠ **この日は2つのセッションが並行して同じリポジトリを触り、矛盾するコミットが出た**（§0-30 の「経緯」節）。
**【次にやること】段階2 P2＝"next time" 型持続の共通機構**（該当7枚＝檻/調査/秘境の社/豊穣/旗艦/上陸部隊/切り裂き魔）。
**相手の片付けでも捨てる経路**が新規に要る。その後 P3〜P7。**
