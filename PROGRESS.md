...
### 【次にやること】
1. **push（ユーザー確認の上で）**＝`sw.js` v62。push 後は Pages/Render の本番反映を実機確認する。
2. その後の未着手拡張（段階1すら未着手）：**同盟／略奪／日の出づる国**。
...
絵（webp）77枚も回収済み（`sw.js` v62）＝**実装済みの全598枚に絵が入っている**。
【次にやること】**push（ユーザー確認の上で）** → 本番反映の実機確認。
...
1. **【最優先】push（ユーザー確認の上で）**＝`sw.js` v62。夜想曲は §0-28 で完成済み
   （全77種が実プレイ可能・絵も入った・全38スイート緑）。push 後は Pages/Render の本番反映を実機確認する。
   その後の未着手拡張（段階1すら未着手＝画像・カタログとも無し）：**同盟／略奪／日の出づる国**。
