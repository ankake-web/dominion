...
最終更新: 2026-08-06 / branch `main`（最新は `git log` で確認）。

**【現在地】§0-24＝UX6件＋実バグ2件（1手もどす／終局後デッキ公開／カード検索／植民地の終了条件／ティアラの出す順）を実装し、
**敵対レビュー確定10件（high 2件＝オンラインUndoの承認バイパス／検索のIME崩れ）も修正して push 済**（`sw.js` v54）。
全34スイート緑（exit 0・prosperity 97・ui 119・server 70）／`verify:e2e` 9/9（webp 371/0）／`verify:visual` はみ出し0／CPU序列 100/64/95。
【次にやること】**発売順の未着手拡張**（夜想曲／移動動物園／同盟／略奪／日の出づる国）。
【次の大仕事】発売順の未着手拡張（夜想曲／移動動物園／同盟／略奪／日の出づる国）。**
...
**新セッションは まず `npm test` を実行し 37スイート・オールグリーン（exit 0・整合性3401件・不変条件8・ルネサンス320件＋UI62件・帝国269件＋UI75件・ランドマーク80件・横型イベント149件・冒険59件＋UI67件・暗黒時代87件＋UI57件・新プロモ165件＋UI22件・繁栄69件・異郷83件＋UI44件・収穫祭107件・ギルド81件＋UI25件・CPU序列 強vs弱100/強vs普通64/普通vs弱95）を確認**してから着手すること。
...
0. ~~mix-all モードのエンジン硬化＋push~~ ✅**完了（§0-23・push済 v52・本番反映確認済）**。
1. **【最優先】次の拡張に着手する**（下記2の候補から選ぶ）。着手前に `docs/adding-cards.md` と §0-23 の「述語を使え」を必読。
2. **次の拡張候補**（着手前に `docs/adding-cards.md` 必読）:
   - ✅**絵の回収は全部完了**＝ルネサンス50枚（2026-07-13）／冒険イベント20種（2026-07-14）／
     帝国ランドマーク21種（2026-07-11）／**帝国イベント13種（2026-08-07）**。
     **全450枚（縦371＋横79）を走査して `asset/art/<id>.png` の欠けゼロを確認済み（2026-08-07）**。
     再生成＝縦型 `CARDS_ONLY=<ids> node tools/build-cards.js`／横型 `CARDS_ONLY=<ids> node tools/build-landscape.js`（このPCのみ）。
