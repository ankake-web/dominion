...
3. **A2＝分割山6組**（混合山モデル＋循環）。**`MIXED_PILE_KEYS = ['ruins','knights','castles']` を6山ぶん拡張する**のが要。
   engine 内に**同じ3つ組のハードコードが7箇所ある**（`gain` の `isMixed`／`trashFromSupplyPile`／`pileKeyOf`／
   `cardCost` の knights/castles 分岐／闇市場の `mixedContents`／`emptyPileCount`／`BLACK_MARKET` の `isMixedPile`）＝
   **一箇所に集約してから広げること**。invariants の tally にも6山を足す（漏れると保存則が誤検知で赤）。
...
**【現在地】§0-29＝新拡張「同盟（Allies）」に着手。①公式ルール研究（多エージェント22体）②段階1（カタログ72種＋webp78枚）
③段階2の A1（好意トークンと Ally 選定の基盤）まで完了。`sw.js` v63・**未push**（5コミット）。
CARD_SETS 未参照＝**実プレイには出ない／本番挙動は不変**（闇市場にも出ないよう `DOM.STAGE1_POOLS` で明示的に塞いだ）。
`npm test` **全38スイート緑**（exit 0・整合性 4184→**4763**・不変条件10）／`verify:e2e` 9/9（webp **505**/0）／
`verify:visual` はみ出し0。実装済みは 縦型505＋横型171＝**676枚**（うち同盟78枚は枠＋文字のみ＝絵は未回収）。
**【次にやること】段階2の A2＝分割山6組**（`MIXED_PILE_KEYS` の3つ組ハードコードが engine 内7箇所にあるので
**まず1箇所に集約してから6山ぶん広げる**）。§0-29 の「次にやること」3番が手順。
